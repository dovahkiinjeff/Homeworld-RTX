...
// not used in command line utility, only in the game
    ...
    /* RTX-0084 map-editor archive browser. These are read-only helpers over the
       already-open BIG precedence table; they never mutate or extract archives. */
    sdword bigArchiveCount(void);
    sdword bigArchiveFileCount(sdword archiveIndex);
    const char *bigArchiveName(sdword archiveIndex);
    bool32 bigArchiveFileName(sdword archiveIndex, sdword fileIndex, char *outName, sdword outNameBytes);
