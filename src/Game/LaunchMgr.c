    ...
    for (insert=0,index=0;index<cmNumCarriers;index++)
    {
        if (cmCarriers[index].owner == universe.curPlayerPtr &&
            (cmCarriers[index].ship->shiptype == Carrier ||
             cmCapturedForeignFactory(cmCarriers[index].ship)))
        {
...
sdword lmLaunchBegin(regionhandle region, smemsize ID, udword event, udword data)
{
    ...
    if ((launchship == NULL) ||
        (((launchship->shiptype != Mothership) && (launchship->shiptype != Carrier)) &&
         !cmCapturedForeignFactory(launchship)))
    {
    ...
    mdPrepareManagerScreen(feScreenFind(LM_LaunchScreen),
                           HW_MANAGER_DOCK_LAUNCH);

    lmIoSaveState = mdManagerDockEnabled() ? FALSE : ioDisable();

    lmBaseRegion = feScreenStart(region, LM_LaunchScreen);  //add new regions as siblings of current one
    mdInstallManagerToggleKeys(lmBaseRegion);
