...
void ccControl(CameraCommand *cameracommand)
{
   ...
   sdword UserAction;
    ...
    /* Shift+F11 owns camera position/focus/chase. Keep only the proven legacy
       mouse yaw/pitch input here; ModernMapEditorCameraUpdate applies the final
       fixed-eye FPS pose immediately after ccControl returns. This prevents
       fleet focus, stack collapse and CameraChase from fighting the editor. */
    if (modernMapEditorActive())
    {
        UserAction = cameraControl(&curentry->remembercam, FALSE);
        if (UserAction) cameracommand->UserControlled = UserAction;
        return;
    }
