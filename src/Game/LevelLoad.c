...
/* CAPTURE / BUILD ALL production hulls can survive hyperspace into levels
   that normally exclude their native race's ships.  Keep every type in the
   factory's native build roster (and the player's active team-color schemes)
   resident so the captured factory remains usable throughout the campaign. */
static void llKeepCapturedFactoryRosterLoaded(Ship *factory, bool8 schemes[])
{
    ShipRace race;
    sdword type;

    if (!cmCapturedForeignFactory(factory))
    {
        return;
    }

    race = cmFactoryBuildRace(factory);
    for (type = FirstShipTypeOfRace[race];
         type <= LastShipTypeOfRace[race];
         type++)
    {
        if (cmCanBuildShipType(factory, (ShipType)type, FALSE))
        {
            SetInfoNeededForShipAndRelatedStaticInfo((ShipType)type, race, TRUE);
            SetTeamColorForShipAndRelatedStaticInfo((ShipType)type, race, schemes);
        }
    }
}
...
void levelPreInit(char *directory,char *pickedMission)
{
    ...
    for (node = singlePlayerGameInfo.ShipsInHyperspace.head; node != NULL; node = node->next)
    {
        ...
        llKeepCapturedFactoryRosterLoaded(ship, teamCol);
    ...
    /* RTX-0084 editor boot: preload all ship/derelict/resource static data after
       mission exclusion scripts have run, so BIG-browser assets are actually
       placeable without mutating the original mission preload script. */
    if (singlePlayerGame && spMapEditorBootActive())
    {
        universeFlagEverythingNeeded();
    }
}
