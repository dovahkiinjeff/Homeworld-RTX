...
/* RTX-0084: the realtime map editor can freeze mission scripting while
   leaving the camera/render loop (and optionally physics) alive. */
static bool32 kasEditorPaused = FALSE;
static bool32 kasEditorSingleStep = FALSE;
...
void kasClose(void)
{
    ...
    kasEditorPaused = FALSE;
    kasEditorSingleStep = FALSE;
}
...
void kasEditorPauseSet(bool32 paused)
{
    kasEditorPaused = paused ? TRUE : FALSE;
    if (!kasEditorPaused)
        kasEditorSingleStep = FALSE;
}

bool32 kasEditorPauseGet(void)
{
    return kasEditorPaused;
}

void kasEditorStepOnce(void)
{
    kasEditorPaused = TRUE;
    kasEditorSingleStep = TRUE;
}
...
void kasExecute(void)
{
    ...
    if (kasEditorPaused && !kasEditorSingleStep)
        return;
    kasEditorSingleStep = FALSE;
...
/* Shift+F11 map-editor enumeration.  Keep LabelledVolumes private to KAS.c,
   but expose stable references to the exact runtime volumes used by KAS. */
//
