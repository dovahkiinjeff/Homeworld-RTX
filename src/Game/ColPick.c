...
{
    if (base != NULL)
        *base = cpPBaseColor != NULL ? *cpPBaseColor : utyBaseColor;
    if (stripe != NULL)
        *stripe = cpPStripeColor != NULL ? *cpPStripeColor : utyStripeColor;
}

void cpModernColorsCommit(color base, color stripe)
{
    cpColorsUpdate(base, stripe);
    if (cpPBaseColor != NULL) *cpPBaseColor = base;
    if (cpPStripeColor != NULL) *cpPStripeColor = stripe;
    cpColorsPicked = TRUE;
}
