...
udword regRegionProcess(regionhandle reg, udword mask)
{
    ...
    if ((regClickedLeft == NULL || regClickedLeft == reg) && bitTest(reg->flags, RPM_MouseEvents))
    {
        ...
        {                                                   //inside region
            sdword wheelSteps = mouseWheelStepsPeek();
            if (wheelSteps > 0 && regFlag(RPE_WheelUp))
            {
                wheelSteps = mouseWheelStepsTake();
                while (wheelSteps-- > 0)
                {
                    regFunctionCall(reg, RPE_WheelUp, 1, &mask);
                }
            }
            else if (wheelSteps < 0 && regFlag(RPE_WheelDown))
            {
                wheelSteps = -mouseWheelStepsTake();
                while (wheelSteps-- > 0)
                {
                    regFunctionCall(reg, RPE_WheelDown, 1, &mask);
                }
            }
