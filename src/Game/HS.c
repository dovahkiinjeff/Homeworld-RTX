...
void hsEnd(Ship* ship, bool32 displayEffect)
{
    ...
    {
        c = utyHyperspaceColor;
    }
    if (hwModernGraphicsIsActive())
    {
        ...
        /* A small floor avoids a single-frame black pop at each state
           transition; the envelope still keeps the gate physically local. */
        envelope = 0.18f + phase * 0.82f;
        emitter.source = HW_MODERN_LIGHT_ION_BEAM;
        emitter.shape = HW_MODERN_LIGHT_SURFACE;
        emitter.position[0] = ship->collInfo.collPosition.x;
        emitter.position[1] = ship->collInfo.collPosition.y;
        emitter.position[2] = ship->collInfo.collPosition.z;
        emitter.direction[0] = ship->rotinfo.coordsys.m13;
        emitter.direction[1] = ship->rotinfo.coordsys.m23;
        emitter.direction[2] = ship->rotinfo.coordsys.m33;
        emitter.color[0] = colUbyteToReal(colRed(c));
        emitter.color[1] = colUbyteToReal(colGreen(c));
        emitter.color[2] = colUbyteToReal(colBlue(c));
        emitter.radius = max(sinfo->rightlength, sinfo->uplength) *
            ship->magnitudeSquared * 1.35f;
        if (emitter.radius < 96.0f) emitter.radius = 96.0f;
        if (emitter.radius > 1600.0f) emitter.radius = 1600.0f;
        emitter.intensity = emitter.radius * 24.0f * envelope;
