...
void LoadPreGameInfo(void)
{
    ...
    if (tutorial != TUTORIAL_ONLY)
    {
        ...
        /* RTX: direct-load migration for already-captured foreign factories.
           PreLoadGame restores the exact static-info set from the save and,
           unlike a hyperspace transition, does not run levelPreInit().  Older
           saves can therefore contain a live P1/P2 mothership while unused
           native build types remain unloaded.  Keep the complete native
           production roster resident before universeStaticInit() runs. */
        if (sp &&
            bitTest(GetShipStaticInfo(P1Mothership, P1)->staticheader.infoFlags, IF_InfoNeeded))
        {
            SetInfoNeededForShipAndRelatedStaticInfo(P1Fighter,           P1, TRUE);
            SetInfoNeededForShipAndRelatedStaticInfo(P1StandardCorvette, P1, TRUE);
            SetInfoNeededForShipAndRelatedStaticInfo(P1MissileCorvette,  P1, TRUE);
            SetInfoNeededForShipAndRelatedStaticInfo(P1IonArrayFrigate,  P1, TRUE);
        }

        /* Same direct-load migration for a captured Kadeshi mothership. */
        if (sp &&
            bitTest(GetShipStaticInfo(P2Mothership, P2)->staticheader.infoFlags, IF_InfoNeeded))
        {
            SetInfoNeededForShipAndRelatedStaticInfo(P2Swarmer,          P2, TRUE);
            SetInfoNeededForShipAndRelatedStaticInfo(P2AdvanceSwarmer,   P2, TRUE);
            SetInfoNeededForShipAndRelatedStaticInfo(P2FuelPod,          P2, TRUE);
            SetInfoNeededForShipAndRelatedStaticInfo(P2MultiBeamFrigate, P2, TRUE);
        }
    }
