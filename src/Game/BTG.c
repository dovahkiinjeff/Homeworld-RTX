...
static GLuint          btgModernStarTexture = 0;
...
void btgReset(void)
{
    ...
    if (btgModernStarTexture != 0)
    {
        glDeleteTextures(1, &btgModernStarTexture);
        btgModernStarTexture = 0;
        trClearCurrent();
    }
...
void btgGetTexture(char* filename, udword* thandle, sdword* width, sdword* height)
{
    ...
    if (fileExists(fullname, 0))
    {
        ...
        *height = (sdword)head.imageHeight;
        ...
        /* Mission backgrounds are stretched across modern high-resolution
           displays. Nearest filtering exposes the original texel grid and
           exaggerates 8-bit gradient bands, so keep this world layer linear. */
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
static real32 btgModernSrgbToLinear(real32 value)
{
    ...
    return (real32)pow(((real64)value + 0.055) / 1.055, 2.4);
}

static real32 btgModernStarRadiance(const char *filename)
{
    if (filename == NULL) return 4.5f;
    if (strstr(filename, "bigstar") != NULL || strstr(filename, "BIGSTAR") != NULL)
        return 8.0f;
    if (strstr(filename, "Neb") != NULL || strstr(filename, "NEB") != NULL)
        return 6.0f;
    if (strstr(filename, "B02") != NULL) return 5.0f;
    if (strstr(filename, "B03") != NULL) return 4.0f;
    return 4.5f;
}

static bool32 btgEnsureModernStarTexture(void)
{
    enum { BTG_MODERN_STAR_SIZE = 128 };
    ubyte *pixels;
    sdword x, y;

    if (btgModernStarTexture != 0) return TRUE;
    pixels = (ubyte *)memAlloc(BTG_MODERN_STAR_SIZE * BTG_MODERN_STAR_SIZE * 4,
                               "modern BTG star texture", 0);
    if (pixels == NULL) return FALSE;

    for (y = 0; y < BTG_MODERN_STAR_SIZE; ++y)
    {
        for (x = 0; x < BTG_MODERN_STAR_SIZE; ++x)
        {
            const real32 nx = (((real32)x + 0.5f) /
                               (real32)BTG_MODERN_STAR_SIZE) * 2.0f - 1.0f;
            const real32 ny = (((real32)y + 0.5f) /
                               (real32)BTG_MODERN_STAR_SIZE) * 2.0f - 1.0f;
            const real32 r2 = nx * nx + ny * ny;
            const real32 core = (real32)exp(-(real64)r2 * 62.0);
            const real32 halo = 0.34f * (real32)exp(-(real64)r2 * 9.5);
            const real32 horizontal = 0.055f *
                (real32)exp(-(real64)(ny * ny) * 520.0) *
                (real32)exp(-(real64)fabs(nx) * 8.0);
            const real32 vertical = 0.055f *
                (real32)exp(-(real64)(nx * nx) * 520.0) *
                (real32)exp(-(real64)fabs(ny) * 8.0);
            const real32 alpha = min(core + halo + horizontal + vertical, 1.0f);
            const sdword index = (y * BTG_MODERN_STAR_SIZE + x) * 4;
            pixels[index + 0] = 255;
            pixels[index + 1] = 255;
            pixels[index + 2] = 255;
            pixels[index + 3] = (ubyte)(alpha * 255.0f + 0.5f);
        }
    }

    glGenTextures(1, &btgModernStarTexture);
    trClearCurrent();
    glBindTexture(GL_TEXTURE_2D, btgModernStarTexture);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA,
                 BTG_MODERN_STAR_SIZE, BTG_MODERN_STAR_SIZE, 0,
                 GL_RGBA, GL_UNSIGNED_BYTE, pixels);
    memFree(pixels);
    return btgModernStarTexture != 0;
}

static void btgRenderModernStars(void)
{
    udword nStar;
    sdword lightingWasEnabled;
    sdword additiveWasEnabled;
    udword textureEnvironment;
    ...
    if (btgHead == NULL || btgStars == NULL || btgTransStars == NULL ||
        btgHead->numStars == 0 || !btgEnsureModernStarTexture())
    {
        return;
    }
    ...
    lightingWasEnabled = rndLightingEnable(FALSE);
    additiveWasEnabled = rndAdditiveBlends(TRUE);
    textureEnvironment = rndTextureEnvironment(RTE_Modulate);
    ...
    if (useVBO)
    {
        ...
        glTexCoordPointer(2, GL_FLOAT, sizeof(GLfloat) * 5,
                          (GLubyte *)sizeof(vector));
        glBindBuffer(GL_ARRAY_BUFFER, 0);
    }
    else
    {
        glVertexPointer(3, GL_FLOAT, sizeof(GLfloat) * 5,
                        (GLubyte *)btgTransStars);
        glTexCoordPointer(2, GL_FLOAT, sizeof(GLfloat) * 5,
                          ((GLubyte *)btgTransStars) + sizeof(vector));
    }

    for (nStar = 0; nStar < btgHead->numStars; ++nStar)
    {
        const real32 radiance = btgModernStarRadiance(btgStars[nStar].filename);
        const real32 fade = btgFade < 0 ? 1.0f :
                            min((real32)btgFade / 255.0f, 1.0f);
        const real32 alpha = ((real32)btgStars[nStar].alpha / 255.0f) * fade;
        const real32 red = btgModernSrgbToLinear(
            (real32)btgStars[nStar].red / 255.0f) * radiance;
        const real32 green = btgModernSrgbToLinear(
            (real32)btgStars[nStar].green / 255.0f) * radiance;
        const real32 blue = btgModernSrgbToLinear(
            (real32)btgStars[nStar].blue / 255.0f) * radiance;
    ...
    rndTextureEnvironment(textureEnvironment);
    rndAdditiveBlends(additiveWasEnabled);
    rndLightingEnable(lightingWasEnabled);
}
...
void btgRender(void)
{
    ...
    /* Campaign skies are now complete, seamless 360-degree environments.
       A successful modern draw owns the background for this frame; the old
       BTG remains loaded only for mission metadata and as a missing-asset/GPU
       compatibility fallback. */
    if (modernMissionBackdropRender((real32)btgFade / 255.0f))
    {
        /* Keep the authored BTG star catalog, but render it as a separate
           analytic HDR layer. Tiny high-radiance stars no longer pass through
           BC6H blocks, so their cores stay round and antialiased instead of
           turning into square compression artifacts. */
        btgRenderModernStars();
        return;
    }
