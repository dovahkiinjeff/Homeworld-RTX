...
//    newship->currentLOD = 0;                                //start at highest LOD by default
    ...
    /* N-LIPS is presentation-only.  Initialize the shared scale to its true
       world-space value so gameplay/effect code cannot observe the legacy
       zero value before the ship's first rendered frame. */
    newship->magnitudeSquared = 1.0f;
#endif
...
bool32 ApplyDamageToTarget(SpaceObjRotImpTarg *target,real32 damagetaken,GunSoundType soundType,sdword damageHow,sdword playerIndex)
{
    ...
    if (singlePlayerGame && opSuperSalvagers &&
        ...
        ((Ship *)target)->playerowner == universe.curPlayerPtr &&
        ((Ship *)target)->shiptype == SalCapCorvette)
    {
        /* Keep the canonical health bar / repair ceiling intact while making
           the craft survive exactly three times as much incoming damage. */
        damagetaken *= (1.0f / 3.0f);
    }
...
void univUpdateRenderList()
{
    ...
    while (objnode != NULL)
    {
        ...
        {                                                   //else check if object should be in render list
            ...
            {
                vecSub(obj->cameraDistanceVector, eyeposition,
                       obj->posinfo.position);
                obj->cameraDistanceSquared =
                    vecMagnitudeSquared(obj->cameraDistanceVector);
