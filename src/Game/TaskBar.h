...
/* Modern HUD policy: keep the Info/command bar attached during normal play.
   Sensors, NIS/cutscene lockout, and the scripted tutorial retain their own
   original visibility control. */
void tbEnsurePersistentInfo(void);

/* Geometry/visibility bridge used by the code-native command surface that
   frames the original center Info bar without intercepting it. */
