...
#ifdef HW_ENABLE_D3D12_BACKEND
    {
        HWModernMapLightEmitter emitter;
        memset(&emitter, 0, sizeof(emitter));
        emitter.type = L_DistantLight;
        emitter.position[0] = lightDefaultLight.position[0];
        emitter.position[1] = lightDefaultLight.position[1];
        emitter.position[2] = lightDefaultLight.position[2];
        emitter.renderDirection[0] = lightDefaultLight.position[0];
        emitter.renderDirection[1] = lightDefaultLight.position[1];
        emitter.renderDirection[2] = lightDefaultLight.position[2];
        emitter.color[0] = lightDefaultLight.diffuse[0];
        emitter.color[1] = lightDefaultLight.diffuse[1];
        emitter.color[2] = lightDefaultLight.diffuse[2];
        emitter.intensity = 1.0f;
        hwModernGraphicsBeginMapLightUpdate();
        hwModernGraphicsSubmitMapLight(&emitter);
        hwModernGraphicsEndMapLightUpdate();
    }
#endif
...
void lightParseHSF(char* fileName)
{
    ...
    if (fileExists(fileName, 0))
    {
        ...
        while (pos <= (sdword)(fileSize - sizeof(HSFLight)))
        {
            ...
            {
                HWModernMapLightEmitter emitter;
                GLfloat convertedPosition[4];
                GLfloat convertedDirection[4];
                real32 heading;
                real32 pitch;
                real32 directionLength;
                memset(&emitter, 0, sizeof(emitter));
                emitter.type = hsfLight->type;
                emitter.position[0] = hsfLight->x;
                emitter.position[1] = hsfLight->y;
                emitter.position[2] = hsfLight->z;
                emitter.orientation[0] = hsfLight->h;
                emitter.orientation[1] = hsfLight->p;
                emitter.orientation[2] = hsfLight->b;

                convertedPosition[0] = hsfLight->x;
                convertedPosition[1] = hsfLight->y;
                convertedPosition[2] = hsfLight->z;
                convertedPosition[3] =
                    hsfLight->type == L_DistantLight ? 0.0f : 1.0f;
                _inplace_glify(convertedPosition);
                emitter.renderPosition[0] = convertedPosition[0];
                emitter.renderPosition[1] = convertedPosition[1];
                emitter.renderPosition[2] = convertedPosition[2];

                /* HSF stores LightWave heading/pitch/bank.  Bank does not
                   alter a light's forward vector.  Convert the authored
                   direction through the same two rotations used for HSF
                   positions so DXR spot and distant lights share the visible
                   scene's coordinate system. */
                /* LightWave HPB values in HSF are radians (the classic maps
                   use pi/2 and 0.837758); cone and edge remain degrees. */
                heading = hsfLight->h;
                pitch = hsfLight->p;
                convertedDirection[0] = fsin(heading) * fcos(pitch);
                convertedDirection[1] = -fsin(pitch);
                convertedDirection[2] = fcos(heading) * fcos(pitch);
                convertedDirection[3] = 0.0f;
                if (hsfLight->type == L_DistantLight)
                {
                    convertedDirection[0] = hsfLight->x;
                    convertedDirection[1] = hsfLight->y;
                    convertedDirection[2] = hsfLight->z;
                }
                _inplace_glify(convertedDirection);
                directionLength = fsqrt(
                    convertedDirection[0] * convertedDirection[0] +
                    convertedDirection[1] * convertedDirection[1] +
                    convertedDirection[2] * convertedDirection[2]);
                if (directionLength > 0.00001f)
                {
                    emitter.renderDirection[0] =
                        convertedDirection[0] / directionLength;
                    emitter.renderDirection[1] =
                        convertedDirection[1] / directionLength;
                    emitter.renderDirection[2] =
                        convertedDirection[2] / directionLength;
                }
                emitter.coneAngle = hsfLight->coneAngle;
                emitter.edgeAngle = hsfLight->edgeAngle;
                emitter.color[0] = (real32)hsfLight->red / 255.0f;
                emitter.color[1] = (real32)hsfLight->green / 255.0f;
                emitter.color[2] = (real32)hsfLight->blue / 255.0f;
                emitter.intensity = hsfLight->intensity;
                hwModernGraphicsSubmitMapLight(&emitter);
            }
#endif
