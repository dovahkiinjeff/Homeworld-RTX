...
bool32 etgErrorRecoverable = FALSE;
bool32 etgErrorEncountered = FALSE;
...
void etgEffAttachParent(bool32 attach);
...
bool32   etgDamageEffectsEnabled = TRUE;          //damage effects on
bool32   etgHitEffectsEnabled = TRUE;             //hit effects on
bool32   etgFireEffectsEnabled = TRUE;            //muzzle flash effects on
bool32   etgBulletEffectsEnabled = TRUE;          //bullet effects on
...
----------------------------------------------------------------------------*/
bool32 etgEffectProfileBaby(udword ID, void *data, struct BabyCallBack *baby)
{
...
//                                    stat->variableSize / 4, "EffectVariables", 0);
    ...
    {
        ((memsize *)effect->variable)[stat->variableOffsets[index]] =
    ...
    {
        ((memsize *)effect->variable)[index] = va_arg(marker, memsize);
    }
    va_end(marker);
...
bool32 etgEffectUpdate(Effect *effect, real32 timeElapsed)
{
...
#ifdef HW_ENABLE_D3D12_BACKEND
static bool32 etgModernStatInLOD(const etgeffectstatic *stat, const etglod *lod)
{
    sdword level;

    if (stat == NULL || lod == NULL)
    {
        return FALSE;
    }
    for (level = 0; level < lod->nLevels; level++)
    {
        if (lod->level[level] == stat)
        {
            return TRUE;
        }
    }
...
static bool32 etgModernNameContains(const char *name, const char *needle)
{
    const char *candidate;
    const char *match;

    if (name == NULL || needle == NULL || *needle == 0)
    {
        return FALSE;
    }
    for (candidate = name; *candidate != 0; candidate++)
    {
        for (match = needle;
             *match != 0 && candidate[match - needle] != 0 &&
             tolower((unsigned char)candidate[match - needle]) ==
...
static bool32 etgModernIsResourceEffect(const etgeffectstatic *stat)
{
    sdword race;
    sdword resourceType;
    ...
    {
        for (resourceType = 0;
             resourceType < ETG_NumberResourceTypes; ++resourceType)
        {
            if (etgModernStatInLOD(
                    stat, etgResourceEffectTable[race][resourceType]))
            {
                return TRUE;
            }
        }
...
/* Resolve the visible resource effect and its DXR emitter from one immutable
   color decision.  The override is player-only; enemy and allied collectors
   retain the authored R1/R2 palette. */
static bool32 etgModernResolveResourceColor(
    const Effect *effect, color *resolvedColor, bool32 *overrideApplied)
{
    const etgeffectstatic *stat;

    *resolvedColor = colRGB(255, 72, 124);
    *overrideApplied = FALSE;
    if (effect == NULL || effect->staticinfo == NULL)
    {
        return FALSE;
    }
    stat = (const etgeffectstatic *)effect->staticinfo;
    if (!etgModernIsResourceEffect(stat))
    {
        return FALSE;
    }
    if (effect->owner != NULL && effect->owner->objtype == OBJ_ShipType)
    {
        const Ship *ship = (const Ship *)effect->owner;
        if (ship->shiprace == R2)
        {
            *resolvedColor = colRGB(80, 188, 255);
        }
        if (utyResourceBeamColorOverride &&
            ship->playerowner == universe.curPlayerPtr)
        {
            *resolvedColor = utyResourceBeamColor;
...
static udword etgModernComputeLightSource(const etgeffectstatic *stat)
{
    sdword race;
    sdword gunSound;
    sdword objectClass;
    ...
    /* Event-table membership is the semantic contract; modded effect names do
       not have to follow Relic's original naming scheme. */
    if (etgModernIsResourceEffect(stat))
    {
        return HW_MODERN_LIGHT_WEAPON;
    }
    for (race = 0; race < NUM_RACES; race++)
    {
        for (gunSound = 0; gunSound < NUM_GUN_SOUND_TYPES; gunSound++)
        {
            if (etgModernStatInLOD(stat,
                    etgGunEventTable[race][gunSound][EGT_GunFire]))
            {
                return HW_MODERN_LIGHT_MUZZLE_FLASH;
            }
        }
    }
    if (etgModernStatInLOD(stat,
            etgSpecialPurposeEffectTable[EGT_BURST_MUZZLE_EFFECT]))
    {
        return HW_MODERN_LIGHT_MUZZLE_FLASH;
    }
    ...
    for (race = 0; race < NUM_RACES; race++)
    {
        for (objectClass = 0; objectClass < NUM_CLASSES; objectClass++)
        {
            for (eventType = 0;
                 eventType < EDT_NumberExplosionTypes; eventType++)
            {
                if (etgModernStatInLOD(stat,
                        etgDeathEventTable[race][objectClass][eventType]))
                {
                    return HW_MODERN_LIGHT_EXPLOSION;
                }
            }
        }
    }
    for (objectClass = 0;
         objectClass < NUM_DERELICTTYPES; objectClass++)
    {
        for (eventType = 0;
             eventType < EDT_NumberExplosionTypes; eventType++)
        {
            if (etgModernStatInLOD(stat,
                    etgDeathEventTableDerelict[objectClass][eventType]))
            {
                return HW_MODERN_LIGHT_EXPLOSION;
            }
        }
    ...
    for (race = 0; race < NUM_RACES; race++)
    {
        ...
        {
            if (etgModernStatInLOD(stat,
                    etgGunEventTable[race][gunSound][EGT_GunHit]) ||
                etgModernStatInLOD(stat,
                    etgGunEventTable[race][gunSound][EGT_BulletDestroyed]))
            {
                return HW_MODERN_LIGHT_EXPLOSION;
            }
        }
    ...
    if (etgModernStatInLOD(stat,
            etgSpecialPurposeEffectTable[EGT_BURST_HIT_EFFECT]) ||
        etgModernStatInLOD(stat,
            etgSpecialPurposeEffectTable[EGT_BURST_EXPLOSION_EFFECT]))
    {
        return HW_MODERN_LIGHT_EXPLOSION;
    }
    for (objectClass = 0; objectClass < NUM_CLASSES; objectClass++)
    {
        for (eventType = 0; eventType < DMG_NumberDamageTypes; eventType++)
        {
            if (etgModernStatInLOD(stat,
                    etgDamageEffectTable[objectClass][eventType]))
            {
                return HW_MODERN_LIGHT_EXPLOSION;
            }
        }
    ...
    for (race = 0; race < NUM_RACES; race++)
    {
        ...
        {
            if (etgModernStatInLOD(stat,
                    etgGunEventTable[race][gunSound][EGT_GunBullet]))
            {
                return etgModernNameContains(stat->name, "ion") ?
                    HW_MODERN_LIGHT_ION_BEAM : HW_MODERN_LIGHT_WEAPON;
            }
        }
    }
    if (etgModernStatInLOD(stat,
            etgSpecialPurposeEffectTable[EGT_BURST_BULLET_EFFECT]) ||
        etgModernStatInLOD(stat,
            etgSpecialPurposeEffectTable[EGT_LIGHTNING_GLOW]))
    {
        return HW_MODERN_LIGHT_WEAPON;
    }
    return 0;
...
static udword etgModernLightSource(const etgeffectstatic *stat)
{
    typedef struct etgModernLightCacheEntry
    {
        const etgeffectstatic *stat;
        udword source;
    } etgModernLightCacheEntry;
    static etgModernLightCacheEntry cache[1024];
    static sdword cacheCount = 0;
    sdword index;
    udword source;

    for (index = 0; index < cacheCount; index++)
    {
        if (cache[index].stat == stat)
        {
            return cache[index].source;
        }
    }
    source = etgModernComputeLightSource(stat);
    if (cacheCount < (sdword)(sizeof(cache) / sizeof(cache[0])))
    {
        cache[cacheCount].stat = stat;
        cache[cacheCount].source = source;
        cacheCount++;
    }
    return source;
}

static void etgSubmitModernLight(const Effect *effect,
                                 bool32 resourceEffect,
                                 color resourceColor)
{
    const etgeffectstatic *stat = (const etgeffectstatic *)effect->staticinfo;
    const SpaceObj *owner = (const SpaceObj *)effect->owner;
    HWModernDynamicLightEmitter emitter = {0};
    real32 age = effect->timeElapsed > 0.0f ? effect->timeElapsed : 0.0f;
    real32 ownerRadius = 12.0f;

    emitter.source = etgModernLightSource(stat);
    if (emitter.source == 0)
    {
        return;
    }
    if (emitter.source == HW_MODERN_LIGHT_MUZZLE_FLASH)
    {
        /* Gun.c now submits the exact barrel-tip event and retains it across
           frames. Avoid double-lighting when an optional ETG sprite exists. */
        return;
    }
    if (owner != NULL && owner->objtype == OBJ_ShipType)
    {
        const Ship *ship = (const Ship *)owner;
        ownerRadius = ship->staticinfo->staticheader.staticCollInfo.collspheresize;
        ownerRadius *= ship->magnitudeSquared;
    }
    else if (owner != NULL && owner->objtype == OBJ_BulletType)
    {
        ownerRadius = ((const Bullet *)owner)->lengthmag * 0.1f;
    }

    emitter.shape = HW_MODERN_LIGHT_POINT;
    emitter.position[0] = effect->posinfo.position.x;
    emitter.position[1] = effect->posinfo.position.y;
    emitter.position[2] = effect->posinfo.position.z;
    emitter.direction[0] = effect->rotinfo.coordsys.m13;
    emitter.direction[1] = effect->rotinfo.coordsys.m23;
    emitter.direction[2] = effect->rotinfo.coordsys.m33;

    if (resourceEffect)
    {
        real32 beamLength = 0.0f;
        real32 resourceRadius = 0.0f;
        real32 beamRadius;
        real32 beamIntensity;
        real32 anchorRadius;
        HWModernDynamicLightEmitter anchor = {0};
        HWModernDynamicLightEmitter contact = {0};

        if (effect->variable != NULL)
        {
            beamLength = ((real32 *)effect->variable)
                [SCALESHIFT(ETG_ResourceLengthParam)];
            resourceRadius = ((real32 *)effect->variable)
                [SCALESHIFT(ETG_ResourceRadiusParam)];
        }
        emitter.shape = HW_MODERN_LIGHT_LINE;
        emitter.endPosition[0] = emitter.position[0] +
            emitter.direction[0] * beamLength;
        emitter.endPosition[1] = emitter.position[1] +
            emitter.direction[1] * beamLength;
        emitter.endPosition[2] = emitter.position[2] +
            emitter.direction[2] * beamLength;
        emitter.color[0] = colUbyteToReal(colRed(resourceColor));
        emitter.color[1] = colUbyteToReal(colGreen(resourceColor));
        emitter.color[2] = colUbyteToReal(colBlue(resourceColor));

        /* The visible harvester beam is broad and extremely bright, but the
           previous DXR contract treated it almost like a thin laser.  Expand
           the lighting volume so the collector hull, the beam itself and the
           nearby resource geometry all sit inside the transport envelope. */
        beamRadius = resourceRadius * 0.38f;
        if (beamRadius < 42.0f) beamRadius = 42.0f;
        if (beamRadius > 196.0f) beamRadius = 196.0f;
        if (beamLength > 0.0f)
        {
            real32 lengthContribution = beamLength * 0.035f;
            if (lengthContribution > beamRadius * 1.35f)
            {
                beamRadius = lengthContribution;
            }
            if (beamRadius > 240.0f) beamRadius = 240.0f;
        }
        beamIntensity = beamRadius * 44.0f;
        if (beamLength > 0.0f)
        {
            beamIntensity += beamLength * 1.6f;
        }
        emitter.radius = beamRadius;
        emitter.intensity = beamIntensity;
        hwModernGraphicsSubmitDynamicLight(&emitter);

        /* Add a strong local emitter at the collector nozzle so the beam
           visibly wraps the source craft instead of only the asteroid end. */
        anchor = emitter;
        anchor.shape = HW_MODERN_LIGHT_POINT;
        anchorRadius = beamRadius * 1.20f;
        if (anchorRadius < 72.0f) anchorRadius = 72.0f;
        if (anchorRadius > 220.0f) anchorRadius = 220.0f;
        anchor.radius = anchorRadius;
        anchor.intensity = beamIntensity * 0.85f;
        hwModernGraphicsSubmitDynamicLight(&anchor);

        /* When the stream terminates on an asteroid or gas cloud, a softer
           contact light helps the receiver glow instead of reading as a dark
           endpoint attached to a bright line. */
        if (beamLength > 8.0f)
        {
            contact = anchor;
            contact.position[0] = emitter.endPosition[0];
            contact.position[1] = emitter.endPosition[1];
            contact.position[2] = emitter.endPosition[2];
            contact.radius = beamRadius * 0.95f;
            if (contact.radius < 56.0f) contact.radius = 56.0f;
            if (contact.radius > 180.0f) contact.radius = 180.0f;
            contact.intensity = beamIntensity * 0.55f;
            hwModernGraphicsSubmitDynamicLight(&contact);
        }
        return;
    }

    if (emitter.source == HW_MODERN_LIGHT_EXPLOSION)
    {
        emitter.color[0] = 1.0f;
        emitter.color[1] = 0.38f;
        emitter.color[2] = 0.08f;
        emitter.radius = ownerRadius > 8.0f ? ownerRadius * 0.55f : 8.0f;
        emitter.intensity = emitter.radius * 28.0f / (1.0f + age * 1.5f);
    }
    else if (emitter.source == HW_MODERN_LIGHT_ION_BEAM ||
             etgModernNameContains(stat->name, "ion"))
    {
        /* Never invent a cyan transport light for a bullet-owned ion effect.
           The visible weapon's authored bulletColor is the lighting color. */
        if (owner != NULL && owner->objtype == OBJ_BulletType)
        {
            const Bullet *ionBullet = (const Bullet *)owner;
            emitter.color[0] = colUbyteToReal(colRed(ionBullet->bulletColor));
            emitter.color[1] = colUbyteToReal(colGreen(ionBullet->bulletColor));
            emitter.color[2] = colUbyteToReal(colBlue(ionBullet->bulletColor));
        }
        else
        {
            emitter.color[0] = 0.12f;
            emitter.color[1] = 0.62f;
            emitter.color[2] = 1.0f;
        }
        /* The exact bullet path now supplies the strong full line plus muzzle
           end-cap.  Keep this auxiliary ETG light local so it reinforces the
           visible FX without creating a second huge hard-edged influence. */
        emitter.radius = ownerRadius > 36.0f ? ownerRadius * 0.24f : 36.0f;
        if (emitter.radius > 260.0f)
        {
            emitter.radius = 260.0f;
        }
        emitter.intensity = emitter.radius * 24.0f /
            (1.0f + age * 2.2f);
    }
    else if (emitter.source == HW_MODERN_LIGHT_MUZZLE_FLASH)
    {
        /* A muzzle flash is tiny as visible geometry but must illuminate a
           useful patch of the firing hull.  The previous three-unit minimum
           usually ended before reaching the nearest GEO triangles. */
        emitter.color[0] = 1.0f;
        emitter.color[1] = 0.58f;
        emitter.color[2] = 0.14f;
        emitter.radius = ownerRadius * 0.25f;
        if (emitter.radius < 24.0f)
        {
            emitter.radius = 24.0f;
        }
        else if (emitter.radius > 160.0f)
        {
            emitter.radius = 160.0f;
        }
        emitter.intensity = emitter.radius * 24.0f /
            (1.0f + age * 12.0f);
    }
    ...
    {
        emitter.color[0] = 1.0f;
        emitter.color[1] = 0.72f;
        emitter.color[2] = 0.22f;
        emitter.radius = ownerRadius > 3.0f ? ownerRadius * 0.07f : 3.0f;
        emitter.intensity = emitter.radius * 10.0f /
            (1.0f + age * 8.0f);
    }
    hwModernGraphicsSubmitDynamicLight(&emitter);
}
...
void etgEffectDraw(Effect *effect)
{
    ...
    vector velInverse;
#ifdef HW_ENABLE_D3D12_BACKEND
    color savedPartEffectColor = partEffectColor;
    bool32 savedPartEffectColorFilterActive = partEffectColorFilterActive;
    real32 savedPartEffectVisibilityScale = partEffectVisibilityScale;
    real32 visibleMuzzleFxScale = 1.0f;
    bool32 suppressVisibleMuzzleFx = FALSE;
    bool32 effectColorOverride = FALSE;
    color effectColor = colBlack;
    color resolvedResourceColor = colBlack;
    bool32 resourceEffect = FALSE;
    bool32 resourceColorOverrideApplied = FALSE;
    bool32 forceFxDepthOcclusion = FALSE;
    GLboolean savedFxDepthTest = GL_TRUE;
    GLint savedFxDepthFunc = GL_LEQUAL;
#endif
...
#ifdef HW_ENABLE_D3D12_BACKEND
    /* Beam/laser FX are translucent, but they are still world geometry and
       must respect opaque ship/asteroid depth.  The legacy particle path can
       inherit a disabled depth test from unrelated FX, which made ion beams
       visibly draw through asteroids. */
    if (effect->owner != NULL && effect->owner->objtype == OBJ_BulletType)
    {
        const Bullet *depthBullet = (const Bullet *)effect->owner;
        if (depthBullet->bulletType == BULLET_Beam ||
            depthBullet->bulletType == BULLET_Laser)
        {
            savedFxDepthTest = glIsEnabled(GL_DEPTH_TEST);
            glGetIntegerv(GL_DEPTH_FUNC, &savedFxDepthFunc);
            glEnable(GL_DEPTH_TEST);
            glDepthFunc(GL_LEQUAL);
            forceFxDepthOcclusion = TRUE;
        }
    }

    /* GunFire and burst-muzzle ETG effects are the visible source flashes for
       ordinary weapons, including the ion-cannon muzzle glow.  Their modern
       cast lights already use the Shift+F12 FX/Emissive multiplier; apply the
       same live multiplier to the legacy raster particles so 0% really means
       no muzzle-source glow and 50%/100% track the cast-light control.  The
       bullet-owned ion beam body is classified separately and is not scaled. */
    if (etgModernLightSource((const etgeffectstatic *)effect->staticinfo) ==
        HW_MODERN_LIGHT_MUZZLE_FLASH)
    {
        visibleMuzzleFxScale = (real32)mainFxLightingStrengthPercent / 100.0f;
        if (visibleMuzzleFxScale < 0.0f) visibleMuzzleFxScale = 0.0f;
        if (visibleMuzzleFxScale > 2.0f) visibleMuzzleFxScale = 2.0f;
        partEffectVisibilityScale = visibleMuzzleFxScale;
        suppressVisibleMuzzleFx = visibleMuzzleFxScale <= 0.0001f;
    }

    /* Resolve the override before either consumer runs.  The exact same color
       is then used for the visible legacy particles and the current frame's
       DXR line light, so transport can never observe a pre-override hue. */
    resourceEffect = etgModernResolveResourceColor(
        effect, &resolvedResourceColor, &resourceColorOverrideApplied);
    etgSubmitModernLight(effect, resourceEffect, resolvedResourceColor);
    if (resourceColorOverrideApplied)
    {
        effectColor = resolvedResourceColor;
        partEffectColor = effectColor;
        partEffectColorFilterActive = TRUE;
        effectColorOverride = TRUE;
    }
    else if (etgModernStatInLOD(
                 (const etgeffectstatic *)effect->staticinfo,
                 etgHyperspaceEffect) &&
             effect->owner != NULL &&
             effect->owner->objtype == OBJ_ShipType &&
             effect->owner->playerowner == universe.curPlayerPtr &&
             utyHyperspaceColorOverride)
    {
        /* The gate particle system is world-space and otherwise copies the
           static ship field below.  Scope a local override so the selected
           player's transition, its visible sprite and its DXR field agree. */
        effectColor = utyHyperspaceColor;
        partEffectColor = effectColor;
        effectColorOverride = TRUE;
    }
#endif
...
#ifdef HW_ENABLE_D3D12_BACKEND
            /* At a true 0% FX setting, do not leave a black/opaque particle
               silhouette behind.  Keep simulation/update alive but skip only
               the visible muzzle-source draw for this frame. */
            if (suppressVisibleMuzzleFx)
            {
                continue;
            }
#endif
...
#ifdef HW_MODERN_DISABLE_NLIPS
                matCopyAndScale(&tempMatrix, &effect->owner->rotinfo.coordsys, 1.0f);
#else
...
#ifdef HW_ENABLE_D3D12_BACKEND
    if (forceFxDepthOcclusion)
    {
        if (savedFxDepthTest) glEnable(GL_DEPTH_TEST);
        else glDisable(GL_DEPTH_TEST);
        glDepthFunc((GLenum)savedFxDepthFunc);
    }
    if (effectColorOverride)
    {
        partEffectColor = savedPartEffectColor;
    }
    partEffectColorFilterActive = savedPartEffectColorFilterActive;
    partEffectVisibilityScale = savedPartEffectVisibilityScale;
#endif
...
----------------------------------------------------------------------------*/
bool32 etgOpcodeScan(struct etgeffectstatic *stat, char *string, ubyte *dest, sdword *destLength)
{
    sdword index;
    sdword strLength;
    char *start, *params;
...
----------------------------------------------------------------------------*/
bool32 etgIsNumber(char *number)
{
    while (*number)
    {
...
----------------------------------------------------------------------------*/
bool32 etgVarCopyScan(struct etgeffectstatic *stat, char *string, ubyte *dest, sdword *destLength)
{
...
----------------------------------------------------------------------------*/
bool32 etgFunctionCallScan(struct etgeffectstatic *stat, char *string, ubyte *dest, sdword *destLength)
{
    ...
    for (index = 0; etgFunctionTable[index].name != NULL; index++)
    {
        ...
        if (strLength > 0)                                  //if found a match
        {
            ...
            opcode->passThis = (bool32)etgFunctionTable[index].passThis;
...
//                  }
            ...
            *destLength = sizeof(etgfunctioncall) + sizeof(memsize) * 2 * (varIndex - 1);
...
----------------------------------------------------------------------------*/
sdword etgNewVariableCreate(char *name, etgeffectstatic *stat, sdword type, bool32 bInitial, memsize initial, sdword size)
{
#if ETG_ERROR_CHECKING
    sdword index;
    ...
    {
        stat->variableSize += sizeof(memsize);
    }
...
----------------------------------------------------------------------------*/
sdword etgVariableTableProcess(ubyte **destOffsets, memsize **destInitData)
{
    memsize *tempBlock, *finalBlock;
    ubyte *tempOffsets, *finalOffsets;
    sdword tableIndex, index, varIndex;

    //create a temp block in the biggest size it could be
    tempBlock = memAlloc((sizeof(ubyte) + sizeof(memsize)) * etgVariableIndex, "TempVarInit", 0);
    ...
    finalBlock = memAlloc((sizeof(ubyte) + sizeof(memsize)) * tableIndex, "VarInitData", NonVolatile);
    finalOffsets = (ubyte *)(&finalBlock[tableIndex]);      //allocate the final blocks
    memcpy(finalBlock, tempBlock, sizeof(memsize) * tableIndex);//copy them over
...
foundThisNewEffect:;
    ...
    if (etgAlternateStructIndex > 0)
    {
        ...
        {
            newStatic->alternateOffsets = memAlloc(sizeof(memsize) * etgTotalOffsets, "AltOffsetTable", NonVolatile);
        }
        else
        {
        ...
        for (index = offset = 0; index < etgAlternateStructIndex; index++)
        {
            ...
            memcpy(newStatic->alternateOffsets + offset, pString,//copy local offset table for this one
                   sizeof(memsize) * etgAlternateTable[index].decision->tableLength);
...
/*-----------------------------------------------------------------------------
    ...
    Return      : pointer to assignment value (35 in above example), or NULL
                    if no initial value.
----------------------------------------------------------------------------*/
char *etgParseVariable(char *params, bool32 *bSetInitial)
{
    char *start, *end;
...
sdword etgNewInteger(struct etgeffectstatic *stat, ubyte *dest, char *opcode, char *params, char *ret)
{
    ...
    {                                                       //if we're creating a constant
        ...
        *((udword *)(etgConstData + stat->constLength)) = initial;
        stat->constLength += sizeof(memsize);
        return(0);
    }
...
sdword etgSubTextureDWORD(struct etgeffectstatic *stat, ubyte *dest, char *opcode, char *params, char *ret)
{
    ...
    if (stat->constLength + SCALESHIFT(12) > ETG_ConstDataPool)
    {
        etgLoadErrorf(ETG, "Exceeded %d bytes of constant data.", ETG_ConstDataPool);
    }
#endif
    ...
    *((memsize *)(etgConstData + stat->constLength)) = SCALECAST(xyInfo);
    stat->constLength += sizeof(memsize);
    *((memsize *)(etgConstData + stat->constLength)) = SCALECAST(flags);
    stat->constLength += sizeof(memsize);
    return(0);
}
...
sdword etgMorphAnimDWORD(struct etgeffectstatic *stat, ubyte *dest, char *opcode, char *params, char *ret)
{
    ...
    if (stat->constLength + SCALESHIFT(8) > ETG_ConstDataPool)
    {
        etgLoadErrorf(ETG, "Exceeded %d bytes of constant data.", ETG_ConstDataPool);
    }
#endif
...
//    function->parameter[0].param = (udword)etgMeshRegistry[etgMeshRegistryIndex];
    function->parameter[0].param = (memsize)etgMeshRegister(param);
...
void etgElseComplete(sdword codeBlock, sdword offset, sdword newOffset, ubyte *userData)
{
    ...
    dbgAssertOrIgnore(opcode->branchTo == MINUS1);
#endif
    //set code length in opcode
...
sdword etgLabelGoto(struct etgeffectstatic *stat, ubyte *dest, char *opcodeString, char *params, char *ret)
{
    ...
    {                                                       //if label not found
        ...
        opcode->branchTo = (memsize)memStringDupe(params);   //save name for later
    }
    else
    {
...
sdword etgSpawnParse(struct etgeffectstatic *stat, ubyte *dest, char *opcodeString, char *params, char *ret)
{
    ...
    call->parameter[0].param = (memsize)spawnEffect;
        ...
        {                                                   //else it's a varaible parameter
            ...
            switch (var->type)
            {
                ...
                default:
                    ...
                    call->parameter[nParams].param = SCALECAST(var->offset);
            }
            call->parameter[nParams].type = (udword)var->type;
        }
        nParams++;
    ...
    call->parameter[1].param = SCALECAST(nParams - 2);
...
void etgCallbackOpen(sdword codeBlock, sdword offset, ubyte *userData)
{
    ...
    call->returnValue = MINUS1;
    call->nParameters = 1;
    call->parameter[0].param = (memsize)etgExecStack.etgCodeBlock[etgParseMode].offset +
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
void etgCallbackClose(memsize codeBlock, sdword offset, sdword newOffset, ubyte *userData)
{
...
sdword etgCreateEffectsParse(struct etgeffectstatic *stat, ubyte *dest, char *opcodeString, char *params, char *ret)
{
    ...
    call->parameter[3].param = SCALECAST(nParams - 4);
    call->nParameters = nParams;
    return(etgFunctionSize(nParams));
}
...
sdword etgFunctionCall(Effect *effect, struct etgeffectstatic *stat, ubyte *opcode)
{
   ...
   memsize returnValue = ((etgfunctioncall *)opcode)->returnValue;
	...
	{                                                       //pass a 'this' pointer
        intParam[currParam] = (memsize)effect;
		currParam++;
	}
...
//        if (!bitTest(flags, SOF_AttachCoordsys))
    ...
    for (index = 0; index < nParams; index++)
    {
        arg[index] = va_arg(argList, memsize);
    }
    va_end(argList);                                        //pass all possible parameters
...
bool32 etgFrequencyExceeded(etgeffectstatic *stat)
{
    ...
    max = stat->nHistoryList;
    historyList = stat->historyList;
...
//           newEffect->posinfo.velocity, effect->posinfo.velocity);
    ...
    return((memsize)newEffect);                              //return pointer to ETG code
}
