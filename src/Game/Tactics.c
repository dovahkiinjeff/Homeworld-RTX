...
//local prototypes
bool32 tacticsShipCanDodge(Ship *ship);
bool32 tacticsShipIsAffectedByTactics(Ship *ship);
bool32 tacticsIsShipLookingForAnyOfThese(Ship *ship,SelectCommand *selection);
bool32 tacticsAreEnemiesNearby(Ship *leadership, Ship *thisship,real32 retaliateZone);
...
bool32 tacticsDeleteMenu(udword num, void *data, struct BabyCallBack *baby)
{
#ifdef DEBUG_TACTICS
...
void tacticsManeuvUpdate(Ship *ship)
{
    ...
    {
        /* Forward/lateral/reverse thrust, rotational authority and turn speed
           all scale together. Reverse thrust is the ship's braking authority,
           so this is a true 2x mobility package rather than a top-speed-only
           cheat. */
        manMult *= 2.0f;
    }
...
/*-----------------------------------------------------------------------------
    ...
    Return      : as above
----------------------------------------------------------------------------*/
bool32 tacticsCheckGuardConditionsDuringAttack(CommandToDo *command)
{
    sdword i;
...
//        }
    ...
    {
    case CLASS_Fighter:
        return(ship->staticinfo->staticheader.maxvelocity*tacticsInfo.MaxVelocityBonus[ship->tactics_ordertype][Tactics_Fighter][ship->tacticstype]*speedmult*superSalvagerMult);
    case CLASS_Corvette:
        return(ship->staticinfo->staticheader.maxvelocity*tacticsInfo.MaxVelocityBonus[ship->tactics_ordertype][Tactics_Corvette][ship->tacticstype]*speedmult*superSalvagerMult);
    default:
        return(ship->staticinfo->staticheader.maxvelocity*speedmult*superSalvagerMult);
    }
