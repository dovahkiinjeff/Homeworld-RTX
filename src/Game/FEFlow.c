...
/*=============================================================================
    Data:
=============================================================================*/
extern bool32 mrMenuDontDisappear;  //TRUE if the menu shouldn't disappear
...
bool32 feRenderEverything = TRUE;
...
void feStaticTextDraw(regionhandle region)
{
    ...
    for (index = 0, source = (char *)atom->pData; index < strCurLanguage; index++)
    {
        source = (char *)memchr(source, 0, SDWORD_Max) + 1;
    }

    dbgAssertOrIgnore(atom->pData);
    fhSave = fontMakeCurrent((fonthandle)atom->attribs);

    available = region->rect.x1 - region->rect.x0;
    if (available < 1) available = 1;
    strncpy(fitted, source != NULL ? source : "", sizeof(fitted) - 1);
    fitted[sizeof(fitted) - 1] = 0;
    length = strlen(fitted);
    while (length > 0 && fontWidth(fitted) > available)
    {
        fitted[--length] = 0;
        clipped = TRUE;
    }
    if (clipped && available > fontWidth("..."))
    {
        while (length > 0 && fontWidth(fitted) + fontWidth("...") > available)
        {
            fitted[--length] = 0;
        }
    ...
    switch (atom->flags & FAM_Justification)
    {
        ...
        case FAM_JustRight:
            ...
            break;
        case FAM_JustCentre:
            x = region->rect.x0 + (available - width) / 2;
            break;
#if FEF_ERROR_CHECKING
        default:
    ...
    y = region->rect.y0 + ((region->rect.y1 - region->rect.y0) - textHeight) / 2;
    if (y < region->rect.y0) y = region->rect.y0;
    if (atom->flags & FAM_DropShadow)
    {
        fontShadowSet((atom->flags & FAM_DropShadow) >> FSB_DropShadow, atom->borderColor);
    }
    fontPrint(x, y, modernUITextColor(region), fitted);
...
{
    modernUIDrawStaticRectangle(region);
}
...
void feBaseRegionDraw(regionhandle region)
{
    ...
    /* RTX-0049: preserve the original FE base-region visibility contract.
       The modern skin used to paint every base region unconditionally, even
       when the authored FIB deliberately marked that base as fully invisible.
       Sensors and HorseRace/loading screens both use transparent utility bases;
       painting those bases is the dark rectangular panel that flashes during
       mode transitions. */
    if (atom == NULL ||
        (!bitTest(atom->flags, FAF_ContentsVisible) &&
         !bitTest(atom->flags, FAF_BorderVisible)))
    {
        return;
    }

    modernUIDrawBase(region);
}
...
//        bitSet(atom->flags,FAF_UseAlpha);
        switch (atom->type)
        {
            ...
            case FA_DecorativeRegion:
                ...
                {
                    regDrawFunctionSet(region, modernUIDrawDecorative);
                }
                break;
...
/* Scale an interface atom and, for normal screens, place it within the
   centred logical UI canvas. Menu coordinates remain relative to the cursor
   or owning region, but their dimensions and hit targets use the same scale. */
static void feResScaleInterfaceAtom(featom *atom, bool32 centred)
{
    if (centred)
    {
        atom->x = uiScalePositionX(atom->x);
        atom->y = uiScalePositionY(atom->y);
    }
    else
    {
        atom->x = uiScaleSize(atom->x);
        atom->y = uiScaleSize(atom->y);
    }

    atom->width = uiScaleSize(atom->width);
    atom->height = uiScaleSize(atom->height);

    if (atom->loadedWidth > 0 && atom->width < 1)
    {
        atom->width = 1;
    }
    if (atom->loadedHeight > 0 && atom->height < 1)
    {
        atom->height = 1;
    }
}
...
void feResRescaleBackground(featom* atom)
{
    ...
    x0 = uiScalePositionX(0);
    y0 = uiScalePositionY(0);
    x1 = uiScalePositionX(UI_LOGICAL_WIDTH);
    y1 = uiScalePositionY(UI_LOGICAL_HEIGHT);

    feResScaleInterfaceAtom(atom, TRUE);
...
----------------------------------------------------------------------------*/
bool32 fePointOnScreen(sdword x, sdword y)
{
    if ((x >= 0) &&
        (y >= 0) &&
        (x < 640) &&
...
/*-----------------------------------------------------------------------------
    ...
    Return      : TRUE or FALSE
----------------------------------------------------------------------------*/
bool32 feAtomOnScreen(featom* atom)
{
...
#ifdef _X86_64
static ubyte *stpcpy_reimplementation(ubyte *dest, const char *src)
{
...
void feScreensLoadAndConvertTo64Bit(char *fileName, void** loadAddress)
{
    ...
    ubyte * oldrootptr = NULL;
    ubyte * newrootptr = NULL;
    ...
    oldfescreen = (fescreen_disk *)(oldrootptr + sizeof(fibfileheader));
...
//    newfescreen = newptr = newrootptr = malloc(sizeof(fescreen) * header->nScreens);
    ...
    newfescreen = (fescreen *)(newrootptr + sizeof(fibfileheader));

    newptr = newrootptr + sizeof(fibfileheader) +
             sizeof(fescreen) * header->nScreens;
...
//        strncpy(newfeatom->hotKeyModifiers, oldfeatom->hotKeyModifiers, 1);
    ...
    newfescreen = (fescreen *)newrootptr;
    oldfescreen = (fescreen_disk *)(oldrootptr + sizeof(fibfileheader));
...
//			screen->atoms[index].contentColor = FIX_ENDIAN_INT_32( screen->atoms[index].contentColor );
        ...
        {                                                   //for each atom in screen
            ...
            if (menuItemsPresent)
            {
                feResScaleInterfaceAtom(&screen->atoms[index], FALSE);
            }
            else
            {
                ...
                else
                {
                    ...
                    feResScaleInterfaceAtom(&screen->atoms[index], TRUE);
                }
            }
...
bool32 feAllScreensReposition(void)
{
    ...
    for (index = 0; index < feScreenIndex; index++)
    {
        ...
        for (a = 0; a < screen->nAtoms; a++)
        {
            ...
            if (hasMenus)
            {
                feResScaleInterfaceAtom(atom, FALSE);
            }
            else if (bitTest(atom->flags, FAF_Background))
            {
...
regionhandle feScreenStart(regionhandle parent, char *screenName)
{
    ...
    modernOwned = modernUIOwnsScreen(screenName);
    modernUIApplyPalette();
    modernUIPrepareScreen(newScreen);
...
//    {                                                       //if it's an 'always on top' screen
        ...
        feStack[feStackIndex].baseRegion = baseRegion = modernOwned ?
            modernUICreateOwnedScreen(parent, newScreen) :
...
#endif
    modernUIDrawMenuItem(region, FECHECKED(atom), bitTest(atom->flags, FAF_Link));
}
...
void feCurrentScreenDelete(void)
{
    ...
    modernOwned = modernUIOwnsScreen(feStack[feStackIndex].screen->name);
    atom = &feStack[feStackIndex].screen->atoms[feStack[feStackIndex].screen->nAtoms - 1];

    for (index = 1; !modernOwned && index < feStack[feStackIndex].screen->nAtoms; index++, atom--)
    {
    ...
    modernUIOwnedScreenDeleted(feStack[feStackIndex].screen);
...
*/
    ...
    for (index = 0; index <= feStackIndex; index++)
    {
        if (feStack[index].baseRegion == baseRegion)
        {
            bool32 modernOwned = modernUIOwnsScreen(feStack[index].screen->name);
            atom = &feStack[index].screen->atoms[feStack[index].screen->nAtoms - 1];

            for (atomindex = 1; !modernOwned && atomindex < feStack[index].screen->nAtoms; atomindex++, atom--)
            {
                if (bitTest(atom->flags, FAF_CallOnDelete))
                {
                    bitSet(atom->status, FAS_OnDelete);
            ...
            modernUIOwnedScreenDeleted(feStack[index].screen);
