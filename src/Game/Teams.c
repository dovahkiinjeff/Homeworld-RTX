...
/* Keep user-authored hull pigments and self-illuminated engine colours on
   separate brightness curves.  Modern colour picking deliberately permits a
   black hull; feeding that value directly into the original trail colourizer
   also made every engine ribbon and its ray-traced emitter black. */
static color teTrailEmissiveSeed(color baseColor, color stripeColor)
{
    color source = baseColor;
    real32 red, green, blue, hue, sat, val;
    ubyte peak = colRed(source);

    if (colGreen(source) > peak)
    {
        peak = colGreen(source);
    }
    if (colBlue(source) > peak)
    {
        peak = colBlue(source);
    }
    ...
    if (peak < 8)
    {
        /* Both paint layers are black, so there is no hue to preserve. */
        source = colRGB(48, 176, 255);
    }

    red = colUbyteToReal(colRed(source));
    green = colUbyteToReal(colGreen(source));
    blue = colUbyteToReal(colBlue(source));
...
static color teTrailEmissiveFloor(color trailColor)
{
    real32 red = colUbyteToReal(colRed(trailColor));
    real32 green = colUbyteToReal(colGreen(trailColor));
    real32 blue = colUbyteToReal(colBlue(trailColor));
    real32 hue, sat, val;
...
#endif
    customTrail = (iTeam == universe.curPlayerIndex &&
                   utyEngineTrailColorOverride);
    if (customTrail)
    {
        trailSeed = utyEngineTrailColor;
    }
    else
    {
        trailSeed = teTrailEmissiveSeed(baseColor, stripeColor);
    }
    for (index = 0; index < TE_NumberTrailColors; index++)
    {
        color adjusted = teColorAdjust(trailSeed, &teTrailColorize[index]);
        teColorSchemes[iTeam].trailColors[index] = customTrail ? adjusted :
            teTrailEmissiveFloor(adjusted);
    }
