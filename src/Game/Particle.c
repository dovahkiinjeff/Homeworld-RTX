...
bool32 partEffectColorFilterActive = FALSE;
real32 partEffectVisibilityScale = 1.0f;
real32 partNLips;

/* Recolor a legacy particle without editing its persistent animation state.
   The strongest animated color channel becomes the selected hue's intensity,
   preserving texture/animation detail and alpha while removing the authored
   palette.  This is deliberately scoped by ETG.c to one effect draw. */
static void partApplyEffectColorFilter(const real32 source[3], real32 result[3])
{
    real32 intensity;

    if (!partEffectColorFilterActive)
    {
        result[0] = source[0];
        result[1] = source[1];
        result[2] = source[2];
    ...
    {
        intensity = source[0];
        if (source[1] > intensity) intensity = source[1];
        if (source[2] > intensity) intensity = source[2];
        if (intensity < 0.0f) intensity = 0.0f;
        if (intensity > 1.0f) intensity = 1.0f;
        result[0] = colUbyteToReal(colRed(partEffectColor)) * intensity;
        result[1] = colUbyteToReal(colGreen(partEffectColor)) * intensity;
        result[2] = colUbyteToReal(colBlue(partEffectColor)) * intensity;
    }

    /* ETG.c scopes this to muzzle-source effects only.  It deliberately
       scales the visible legacy raster FX with the same live Shift+F12
       FX/Emissive strength that already controls their cast illumination. */
    result[0] *= partEffectVisibilityScale;
    result[1] *= partEffectVisibilityScale;
    result[2] *= partEffectVisibilityScale;
}
...
static void drawBox(GLfloat size, GLenum type, particle *p, bool32 alpha)
{
    ...
    GLfloat v[8][3];
    real32 filteredColor[3];
    GLint i;

    partApplyEffectColorFilter(p->icolor, filteredColor);
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        {
            glNormal3fv(&n[i][0]);
            glColor4f(filteredColor[0], filteredColor[1], filteredColor[2], p->icolor[3]);
        ...
        {
            glNormal3fv(&n[i][0]);
            glColor3f(filteredColor[0], filteredColor[1], filteredColor[2]);
            glVertex3fv(&v[faces[i][0]][0]);
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
void partBillboardEnable(vector *v, bool32 isWorldspace)
{
    if (isWorldspace)
    {
        //enable billboarding
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
void partBillboardDisable(void)
{
...
udword partRenderBillSystem(udword n, particle* p, udword flags,
                            ...
                            billSystem* bpart, bool32 isWorldspace)
{
    udword i, hits;
    vector pos;
    color col;
...
*/
    ...
    for (i = hits = 0; i < n; i++, p++)
    {
        ...
        partApplyEffectColorFilter(saturatedBias, saturatedBias);
        ...
        partApplyEffectColorFilter(tmpColors, tmpColors);

        if (currentTex == TR_Invalid)
        {
            col = colRGBA(colRealToUbyte(tmpColors[0]), colRealToUbyte(tmpColors[1]),
                          colRealToUbyte(tmpColors[2]), colRealToUbyte(tmpColors[3]));
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
void partMeshMaterialPrepare(particle *p, trhandle currentTex, materialentry *material, bool32 alpha)
{
    texreg *reg;
    GLfloat attribs[4];
    real32 filteredColor[3];
    ...
    {                                           //if particle is lit
        ...
        attribs[0] = filteredColor[0] * ambientFactor;
        attribs[1] = filteredColor[1] * ambientFactor;
        attribs[2] = filteredColor[2] * ambientFactor;
        attribs[3] = 1.0f;
        glMaterialfv(face, GL_AMBIENT, attribs);
        attribs[0] = filteredColor[0] * specularFactor;
        attribs[1] = filteredColor[1] * specularFactor;
        attribs[2] = filteredColor[2] * specularFactor;
        glMaterialfv(face, GL_SPECULAR, attribs);
        attribs[0] = filteredColor[0];
        attribs[1] = filteredColor[1];
        attribs[2] = filteredColor[2];
...
void undoPseudoBillboardMatrix(void)
{
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
void partMeshOrient(particle* p, bool32 bRescaleNormal, meshSystem* meshPart)
{
    vector velocity;
    hmatrix velHMatrix;
    matrix velMatrix;
    ...
    for (i = hits = 0; i < n; i++, p++)
    {
        ...
        if ((mesh != NULL) && (mesh != (meshdata*) 0xffffffff) && (mesh != (meshdata*) 0x7fffffff))
        {
            ...
            if (bitTest(flags, PART_ALPHA))
            {
                ...
                {
                    meshSetSpecular(0, RUB(filteredColor[0]), RUB(filteredColor[1]), RUB(filteredColor[2]), RUB(p->icolor[3]));
                }
            }
            else
            {
                ...
                if (usingShader && g_SpecHack)
                {
                    meshSetSpecular(0, RUB(filteredColor[0]), RUB(filteredColor[1]), RUB(filteredColor[2]), 255);
                }
            }
            ...
            if ((p->bias[0] + p->bias[1] + p->bias[2]) > 0.0f)
            {
                if (hsColor && !partEffectColorFilterActive)
                {
                ...
                partApplyEffectColorFilter(csat, csat);
...
psysPtr partCreationHelper(particleType t, udword n, udword dist)
{
    ...
    isWorldspace = (bool32)bitTest(pat.flags, PART_WORLDSPACE);
