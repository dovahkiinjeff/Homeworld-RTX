...
void uicButtonDraw(regionhandle reg)
{
    modernUIDrawButton(reg, FALSE, FALSE);
    return;
...
void uicToggleDraw(regionhandle reg)
{
    modernUIDrawButton(reg, FECHECKED(((featom *)reg->userID)), FALSE);
    return;
...
void uicScrollBarSynchronizeGeometry(scrollbarhandle scrollbar)
{
    ...
    cachedX = scrollbar->thumbreg.x0 - (scrollbar->isVertical ? 0 : 4);
    cachedY = scrollbar->thumbreg.y0 - (scrollbar->isVertical ? 4 : 0);
    deltaX = scrollbar->reg.rect.x0 - cachedX;
    deltaY = scrollbar->reg.rect.y0 - cachedY;
    if (deltaX == 0 && deltaY == 0)
    {
        return;
    }
    scrollbar->thumbreg.x0 += deltaX;
    scrollbar->thumbreg.x1 += deltaX;
    scrollbar->thumbreg.y0 += deltaY;
    scrollbar->thumbreg.y1 += deltaY;
    scrollbar->thumb.x0 += deltaX;
    scrollbar->thumb.x1 += deltaX;
    scrollbar->thumb.y0 += deltaY;
    scrollbar->thumb.y1 += deltaY;
}
...
udword uicScrollBarProcess(regionhandle region, smemsize ID, udword event, udword data)
{
    ...
    switch (ID)
    {
        case UIC_ScrollbarTab:
            ...
            uicScrollBarSynchronizeGeometry(shandle);
            switch (event)
            {
                case RPE_ReleaseLeft:
...
void uicScrollBarButtonDraw(regionhandle reg)
{
    modernUIDrawScrollBarButton(reg);
    return;
...
{
    modernUIDrawListSelection(rect);
}
...
{        /*
    ...
    erase.x1 -= 3;
           */
    //primRectSolid2(&erase, colBlack);
    modernUIDrawButton(reg, FECHECKED(((featom *)reg->userID)), TRUE);
    return;
    if (feStack[feStackIndex].screen != ((buttonhandle)reg)->screen)
    {
        if (FECHECKED(((featom *)reg->userID)))
...
//    udword timerInt, flashRateInt;
    ...
    modernUIDrawTextEntryFrame(reg);

    if (FALSE && !bitTest(entry->textflags,UICTE_NoTextures))
    {
        if (feStack[feStackIndex].screen != ((buttonhandle)reg)->screen)
        {
