...
bool32 CollectResources(Ship *ship,Resource *resource)
{
    ...
    if (UNIVERSE_WOODPECKER(shipstatic->harvestRate, 0))
    {
        ...
        {
            sdword multiplier = 1;
            sdword harvestAmount = shipstatic->harvestAmount;

            /* RTX-0075: leave every authored/static resource value untouched,
               but consume source RUs at 1x-4x the normal harvesting rate.
               Dock transfer applies the same multiplier to the collected cargo,
               so a deposit yields exactly 1x-4x total RUs while collectors fill
               at the selected rate instead of merely rewriting asset values. */
            if (singlePlayerGame)
            {
                multiplier = (sdword)opResourceMultiplier;
                if (multiplier < 1) multiplier = 1;
                if (multiplier > 4) multiplier = 4;
                harvestAmount *= multiplier;
            }

            eat = (harvestAmount <= resource->resourcevalue)
                ? harvestAmount : resource->resourcevalue;
            ship->resources += eat;
