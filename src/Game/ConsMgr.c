...
#define CM_DefaultFont          "simplixssk_13.hff"
...
#define CM_PausedColor          colRGB(143,157,157)
#define CM_SelectedColorSolid   colRGB(52,42,25)
#define CM_SelectedColorOutline colRGB(222,162,62)
#define CM_BackgroundColor      colRGB(15,25,32)
...
static bool32 cmDockFactoryVisible(const Ship *ship);
static sdword cmDockFactoryCount(void);
static Ship *cmDockFactoryAt(sdword index);
static sdword cmDockFactoryIndexOf(const Ship *ship);
static void cmDockFactoryInstall(regionhandle parent);
static sdword cmDockFactoryProcess(regionhandle region, sdword ID, udword event, udword data);
static void cmDockFactoryDraw(regionhandle region);
...
/* Docked Build Manager factory selector.  The legacy FIB only exposed the
   player Mothership plus four Carrier slots.  Modern capture/build support
   needs the actual owned production-factory list instead: the player
   mothership, ordinary carriers, and captured native foreign factories such
   as P1Mothership. */
#define CM_DOCK_FACTORY_COLUMNS       2
#define CM_DOCK_FACTORY_ROWS          5
#define CM_DOCK_FACTORY_VISIBLE       (CM_DOCK_FACTORY_COLUMNS * CM_DOCK_FACTORY_ROWS)
#define CM_DOCK_FACTORY_BUTTON        30
#define CM_DOCK_FACTORY_GAP           4
#define CM_DOCK_FACTORY_HEADER        16

static regionhandle cmDockFactoryRegion = NULL;
static Ship *cmDockSelectedFactory = NULL;
static sdword cmDockFactoryScroll = 0;
...
/* Forward declaration used by the docked-manager migration repair below. */
shipsinprogress *cmFindFactory(struct Ship *ship);

static bool32 cmDockFactoryVisible(const Ship *ship)
{
    if (ship == NULL || ship->playerowner == NULL ||
        ship->playerowner != universe.curPlayerPtr || ship->staticinfo == NULL ||
        !ship->staticinfo->canBuildShips)
    {
        return FALSE;
    }

    /* Native factories are always visible.  Captured foreign production
       hulls remain registered persistently, but CAPTURE / BUILD ALL controls
       whether they are exposed as build factories right now. */
    if (ship->staticinfo->shiprace == ship->playerowner->race)
    {
        return TRUE;
    }
    return cmCaptureBuildAllFactoryEnabled(ship);
}

static void cmDockEnsureOwnedFactoriesRegistered(void)
{
    Node *node;

    /* Save games made before captured-factory support can contain a perfectly
       valid persistent production hull (including P1Mothership) without a
       corresponding construction-manager entry.  Docking and launching still
       work in that state, which is why the bug presents as a missing Build
       Manager button only.  Repair that legacy state from the authoritative
       live ship list whenever the docked Build Manager is used. */
    for (node = universe.ShipList.head; node != NULL; node = node->next)
    {
        Ship *ship = (Ship *)listGetStructOfNode(node);

        if (cmDockFactoryVisible(ship) && cmFindFactory(ship) == NULL)
        {
            cmAddFactory(ship, ship->staticinfo->canBuildBigShips);
        }
    }
}
...
static sdword cmDockFactoryCount(void)
{
    ...
    cmDockEnsureOwnedFactoriesRegistered();

    for (node = listofShipsInProgress.head; node != NULL; node = node->next)
    {
        shipsinprogress *factory = (shipsinprogress *)listGetStructOfNode(node);
        if (factory != NULL && cmDockFactoryVisible(factory->ship))
        {
            count++;
        }
    }
...
static Ship *cmDockFactoryAt(sdword wanted)
{
    Node *node;
    sdword index = 0;
...
static void cmDockFactoryNormalizeSelection(void)
{
    sdword count = cmDockFactoryCount();
    sdword selected = cmDockFactoryIndexOf(cmDockSelectedFactory);
    sdword maxScroll;

    if (selected < 0)
    {
        cmDockSelectedFactory = cmDockFactoryAt(0);
        selected = cmDockFactoryIndexOf(cmDockSelectedFactory);
    }

    maxScroll = max(0, count - CM_DOCK_FACTORY_VISIBLE);
    if (maxScroll & 1)
    {
        maxScroll++;
    }
    if (cmDockFactoryScroll > maxScroll)
    {
        cmDockFactoryScroll = maxScroll;
    }
    if (cmDockFactoryScroll < 0)
    {
        cmDockFactoryScroll = 0;
    }
    cmDockFactoryScroll &= ~1;

    if (selected >= 0)
    {
        if (selected < cmDockFactoryScroll)
        {
            cmDockFactoryScroll = selected & ~1;
        }
        else if (selected >= cmDockFactoryScroll + CM_DOCK_FACTORY_VISIBLE)
        {
            cmDockFactoryScroll = (selected - CM_DOCK_FACTORY_VISIBLE + 2) & ~1;
        }
    }
}

static void cmDockFactoryButtonRect(regionhandle region, sdword visibleIndex, rectangle *rect)
{
    sdword column = visibleIndex % CM_DOCK_FACTORY_COLUMNS;
    sdword row = visibleIndex / CM_DOCK_FACTORY_COLUMNS;
    sdword button = uiScaleSize(CM_DOCK_FACTORY_BUTTON);
    sdword gap = uiScaleSize(CM_DOCK_FACTORY_GAP);
    sdword header = uiScaleSize(CM_DOCK_FACTORY_HEADER);

    rect->x0 = region->rect.x0 + column * (button + gap);
    rect->y0 = region->rect.y0 + header + row * (button + gap);
    rect->x1 = rect->x0 + button;
    rect->y1 = rect->y0 + button;
}

static char *cmDockFactoryTag(const Ship *ship)
{
    ...
    {
        case R1: return ship->shiptype == Mothership ? "MS" : "R1";
        case R2: return ship->shiptype == Mothership ? "MS" : "R2";
        case P1: return "P1";
        case P2: return "P2";
        default: return "FX";
    }
...
static void cmDockFactoryDrawIcon(rectangle *button, Ship *ship, bool32 selected, bool32 hovered)
{
    rectangle icon = *button;
    fonthandle oldfont;
    char *tag;
    sdword textWidth;
    ShipRace playerRace = universe.curPlayerPtr->race;
    udword useTexture = (ship != NULL &&
                         (ship->shiptype == Mothership ||
                          ship->shiptype == P1Mothership ||
                          ship->shiptype == P2Mothership)) ? 0 : 1;
    color outline = selected ? CM_SelectedColorOutline :
                    (hovered ? colRGB(122, 132, 122) : colRGB(58, 76, 84));

    primRectSolid2(button, CM_BackgroundColor);
    primRectOutline2(button, selected ? 2 : 1, outline);

    icon.x0 += uiScaleSize(3);
    icon.y0 += uiScaleSize(3);
    icon.x1 -= uiScaleSize(3);
    icon.y1 -= uiScaleSize(3);

    if (playerRace < MAX_RACES && cmShipImage[playerRace][useTexture] != NULL)
    {
        if (cmPaletted)
        {
            trPalettedTextureMakeCurrent(cmShipTexture[playerRace][useTexture],
                                         cmShipImage[playerRace][useTexture]->palette);
        }
        else
        {
            trRGBTextureMakeCurrent(cmShipTexture[playerRace][useTexture]);
        }
        primRectSolidTextured2(&icon);
    }

    tag = cmDockFactoryTag(ship);
    oldfont = fontMakeCurrent(cmShipListFont);
    textWidth = fontWidth(tag);
    fontPrint(button->x1 - textWidth - uiScaleSize(2),
              button->y1 - fontHeight(NULL) - uiScaleSize(1),
              selected ? CM_SelectedColorOutline : FEC_ListItemStandard, tag);
...
static void cmDockFactoryDraw(regionhandle region)
{
    ...
    if (!mdManagerDockEnabled() || region == NULL)
    {
        return;
    }

    cmDockFactoryNormalizeSelection();
    count = cmDockFactoryCount();

    oldfont = fontMakeCurrent(cmShipListFont);
    fontPrint(region->rect.x0, region->rect.y0,
              CM_ClassHeadingTextColor, "FACTORIES");
    fontMakeCurrent(oldfont);

    for (slot = 0; slot < CM_DOCK_FACTORY_VISIBLE; slot++)
    {
        sdword index = cmDockFactoryScroll + slot;
        Ship *ship;
        rectangle button;
        bool32 hovered;
        ...
        ship = cmDockFactoryAt(index);
        if (ship == NULL)
        {
            continue;
        }
        cmDockFactoryButtonRect(region, slot, &button);
        hovered = mouseCursorX() >= button.x0 && mouseCursorX() < button.x1 &&
                  mouseCursorY() >= button.y0 && mouseCursorY() < button.y1;
        cmDockFactoryDrawIcon(&button, ship, ship == cmDockSelectedFactory, hovered);
    }

    if (count > CM_DOCK_FACTORY_VISIBLE)
    {
        char page[24];
        oldfont = fontMakeCurrent(cmShipListFont);
        sprintf(page, "%d-%d/%d", cmDockFactoryScroll + 1,
                min(count, cmDockFactoryScroll + CM_DOCK_FACTORY_VISIBLE), count);
        fontPrint(region->rect.x0,
                  region->rect.y1 - fontHeight(NULL),
                  FEC_ListItemStandard, page);
        fontMakeCurrent(oldfont);
    }
}
...
static sdword cmDockFactoryProcess(regionhandle region, sdword ID, udword event, udword data)
{
    ...
    {
        sdword maxScroll = max(0, count - CM_DOCK_FACTORY_VISIBLE);
        if (maxScroll & 1)
        {
            maxScroll++;
        }
        if (event == RPE_WheelUp)
        {
            cmDockFactoryScroll = max(0, cmDockFactoryScroll - CM_DOCK_FACTORY_COLUMNS);
        }
        else
        {
            cmDockFactoryScroll = min(maxScroll,
                                      cmDockFactoryScroll + CM_DOCK_FACTORY_COLUMNS);
        }
        cmDockFactoryScroll &= ~1;
    ...
    for (slot = 0; slot < CM_DOCK_FACTORY_VISIBLE; slot++)
    {
        ...
        cmDockFactoryButtonRect(region, slot, &button);
        if (mouseCursorX() < button.x0 || mouseCursorX() >= button.x1 ||
            mouseCursorY() < button.y0 || mouseCursorY() >= button.y1)
        {
            continue;
        }
        ...
        ship = cmDockFactoryAt(index);
        if (ship != NULL && ship != cmDockSelectedFactory)
        {
            cmDockSelectedFactory = ship;
            cmUpdateFactory(ship);
            cmUpperIndex = 0;
            cmArrowIndex = -1;
            updateShipView();
            cmDirtyShipInfo();
            regRecursiveSetDirty(cmBaseRegion);
        }
        break;
    }
    return 0;
...
static void cmDockFactoryInstall(regionhandle parent)
{
    ...
    cmDockFactoryRegion = NULL;
    if (!mdManagerDockEnabled() || parent == NULL)
    {
        return;
    }

    mdGetPanelRect(&panel);
    /* Build Manager is visually compacted to the rightmost 300 logical pixels
       of the manager dock.  Keep the factory bank flush with that compact
       layout rather than the old 360-pixel geometry. */
    rect.x0 = panel.x1 - uiScaleSize(74);
    rect.x1 = panel.x1 - uiScaleSize(8);
    rect.y0 = panel.y0 + uiScaleSize(126);
    rect.y1 = panel.y0 + uiScaleSize(362);

    cmDockFactoryRegion = regChildAlloc(parent, (smemsize)&cmDockFactoryRegion,
                                        rect.x0, rect.y0,
                                        rect.x1 - rect.x0, rect.y1 - rect.y0,
                                        0,
                                        RPE_PressLeft | RPE_WheelUp |
                                        RPE_WheelDown | RPE_DrawEveryFrame);
    regDrawFunctionSet(cmDockFactoryRegion, cmDockFactoryDraw);
    regFunctionSet(cmDockFactoryRegion, (regionfunction)cmDockFactoryProcess);
    regSiblingMoveToFront(cmDockFactoryRegion);
    cmDockFactoryNormalizeSelection();
}
...
static bool32 cmBuildRosterRaceSupported(ShipRace race)
{
    return race == R1 || race == R2 || race == P1 || race == P2;
}

static bool32 cmIsForeignProductionShip(const Ship *ship)
{
    if (!singlePlayerGame || ship == NULL || ship->playerowner == NULL ||
        ship->playerowner != universe.curPlayerPtr)
    {
        return FALSE;
    }
    if (!ship->staticinfo->canBuildShips ||
        !cmBuildRosterRaceSupported(ship->staticinfo->shiprace))
    {
        return FALSE;
    }
    return ship->staticinfo->shiprace != ship->playerowner->race;
}

bool32 cmCapturedForeignFactory(const Ship *ship)
{
    /* Intrinsic identity of an already player-owned foreign production hull.
       This remains true even when CAPTURE / BUILD ALL is switched off so the
       ship can keep its docking/launch state, queued jobs can finish, and its
       native roster can stay resident for a later re-enable. */
    return cmIsForeignProductionShip(ship);
}

bool32 cmCaptureBuildAllFactoryEnabled(const Ship *ship)
{
    /* The Gameplay option is authoritative for exposing foreign production
       in the Build Manager and for accepting new exceptional captures. */
    return opCaptureBuildAll != 0 && cmCapturedForeignFactory(ship);
}

ShipRace cmFactoryBuildRace(const Ship *ship)
{
    if (ship != NULL && ship->playerowner != NULL)
    {
        if (cmCapturedForeignFactory(ship))
        {
            return ship->staticinfo->shiprace;
        }
        return ship->playerowner->race;
    }
    return R1;
}
...
void cmAddFactory(struct Ship *ship,bool32 canBuildBigShips)
{
    ...
    if (ship->shiptype == Carrier || cmIsForeignProductionShip(ship))
    {
        sdword i;
        bool32 remembered = FALSE;

        /* Keep the original deterministic Carrier list in multiplayer.
           In single-player it also doubles as the factory/launch selector
           list for captured native production hulls (P1Mothership, foreign
           Carriers, etc.).  Legacy-save repair may call cmAddFactory for a
           ship that already has a selector record, so never duplicate it. */
...
void cmRemoveFactory(struct Ship *ship)
{
    ...
    if (cmDockSelectedFactory == ship)
    {
        cmDockSelectedFactory = NULL;
    }
    if (sinprogress != NULL)
    {
...
*/
        ...
        while (node != NULL)
        {
            ...
            {
                for (index = 0, progress = &sinprogress->progress[0]; index < TOTAL_NUM_SHIPS; index++, progress++)
...
//                                clWrapRUTransfer(&universe.mainCommandLayer,0,universe.curPlayerPtr->playerIndex,progress->info->buildCost,RUTRANS_BUILTSHIP);
                                ...
                                {
                                    //xxx
                                    /* The queued static-info record is the
                                       authority for native race.  This also
                                       lets an already-started foreign build
                                       finish correctly if CAPTURE / BUILD ALL
                                       is switched off before completion. */
...
void cmUpdateShipAvailable(void)
{
    ...
    bool32 captureBuildAll = (curshipsInProgress != NULL) &&
                             cmCaptureBuildAllFactoryEnabled(curshipsInProgress->ship);
    ...
    for (index = 0; cmShipsAvailable[index].nJobs != -1; index++)
    {
        ...
        {
            if ((captureBuildAll ||
                 rmCanBuildShip(universe.curPlayerPtr, cmShipsAvailable[index].type)) &&
...
{                                                           //close the construction manager
    ...
    // The main view never stopped in docked mode; this is harmless and keeps
    // the legacy/tutorial close path explicit.
    mrRenderMainScreen = TRUE;

    /* play the exit sound */
    soundEvent(NULL, UI_ManagerExit);
    if (!mdManagerDockEnabled())
    {
...
void cmCancelJobs(char *string, featom *atom)
{
    ...
    for (index = 0; index < TOTAL_NUM_SHIPS; index++, progress++)
    {                                                   //for all classes of ships
...
void cmPauseJobs(char *string, featom *atom)
{
    ...
    //all paused?
    progress = &curshipsInProgress->progress[0];
    for (index = 0; (index < TOTAL_NUM_SHIPS) && allpaused; index++, progress++)
    {                                                   //for all classes of ships
        if (progress->selected)
        {
            if (!progress->paused)
...
void cmShipCostsDraw(featom *atom, regionhandle region)
{
    ...
    primLine2(rect->x0, rect->y0, rect->x0, rect->y1, colRGB(86, 111, 120));
...
bool32 cmClassCapCanCreateShip(ShipType shipType, shipsinprogress *factory, shipavailable *cmShipsAvail)
{
    ...
    shipRace = cmFactoryBuildRace(factory->ship);
    shipStaticInfo = GetShipStaticInfo(shipType, shipRace);
    shipClass = cmClassCapClassRemap(shipType, shipStaticInfo->shipclass);
    ...
    {
        shipStaticInfo = cmShipsAvail[index].info;
        if (cmShipsAvail[index].nJobs > 0 && shipStaticInfo != NULL)
    ...
    for (index = 0; index < TOTAL_NUM_SHIPS; index++)
    {
        ...
        {
            shipStaticInfo = factory->progress[index].info;
            dbgAssertOrIgnore(shipStaticInfo != NULL);
            if (shipStaticInfo != NULL &&
                cmClassCapClassRemap(shipStaticInfo->shiptype, shipStaticInfo->shipclass) == shipClass)
            {                                               //if this job the same class as the one we want to build
    ...
    //find all ships not yet started due to network lag.  This path is
    //multiplayer-only; use the safe lookup because foreign-race ShipType
    //values are outside the standard race's contiguous static-info table.
    for (index = 0; index < TOTAL_NUM_SHIPS; index++)
    {
        if (shiplagtotals[index] > 0)
        {
            shipStaticInfo = GetShipStaticInfoSafe(index, shipRace);
            if (shipStaticInfo != NULL &&
                cmClassCapClassRemap(shipStaticInfo->shiptype, shipStaticInfo->shipclass) == shipClass)
            {
                if (!shipsQueuedUp[index])
...
void cmShipNumbersDraw(featom *atom, regionhandle region)
{
    ...
    for (index=startind;cmShipsAvailable[index].nJobs!=-1; index++)
    {
             ...
             (cmShipsAvailable[index].itemstat==STAT_CANBUILD) )
        {
            ...
            x = (rect->x0 + rect->x1 - fontWidthf("%d", numShips)) / 2;
    ...
    if ( (event == RPE_PressLeft || event == RPE_DoubleLeft) && (index!=-1) )
    {
        ...
        else
        {
            ...
            else
            {
                ...
                kprogress = progress;
                progress = &curshipsInProgress->progress[0];
                for (i = 0; i < TOTAL_NUM_SHIPS; i++, progress++)
                {                                                   //for all classes of ships
...
void cmShipInfoDraw(featom *atom, regionhandle region)
{
    ...
    /* cmMaxIndex used to be the fixed 640x480-era value 18.  Once the build
       manager became a resizable in-game panel, that made the scrollbar stop
       early whenever fewer than 18 rows were physically visible.  Derive the
       page size from the live list region so the final ship/class rows are
       always reachable at every UI scale. */
    lineHeight = fontHeight(" ") + CM_ASInterSpacing;
    if (lineHeight < 1) lineHeight = 1;
    visibleLines = (region->rect.y1 - region->rect.y0 - CM_ASMarginTop) /
                   lineHeight;
    if (visibleLines < 1) visibleLines = 1;
    if (NUMBER_SHIPS_AVAILABLE > 0 &&
        visibleLines > NUMBER_SHIPS_AVAILABLE)
        visibleLines = NUMBER_SHIPS_AVAILABLE;
    if (NUMBER_SHIPS_AVAILABLE > 0 &&
        cmMaxIndex != (uword)visibleLines)
    {
        sdword maxUpper = NUMBER_SHIPS_AVAILABLE - visibleLines;
        if (maxUpper < 0) maxUpper = 0;
        cmMaxIndex = (uword)visibleLines;
        if ((sdword)cmUpperIndex > maxUpper)
            cmUpperIndex = (uword)maxUpper;
        if (g_shandle != NULL)
            scAdjustThumbwheel(g_shandle, cmUpperIndex, cmMaxIndex,
                               (uword)NUMBER_SHIPS_AVAILABLE);
    }
...
*/
            ...
            if (progress->selected)
            {
                ...
                rect.x1 = region->rect.x1 - CM_ProgressMarginRight + cmSelectRightAdjust;
                rect.y1 = y + fontHeight(" ") + CM_ASInterSpacing - 1 + cmSelectBottomAdjust;
            ...
            {                                                   //if working on this class
                ...
                rect.y1 = y + fontHeight(" ") + CM_ASInterSpacing - 2 + cmSelectBottomAdjust;
                rect.x1 = rect.x0 - 1 + cmSelectRightAdjust + percent * (region->rect.x1 -
                    region->rect.x0 - CM_ProgressMarginLeft - CM_ProgressMarginRight) / 100;
            ...
            if (progress->paused)
            {
                ...
                rect.x1 = region->rect.x1 - CM_ProgressMarginRight + 2;
...
void cmNumberRUsDraw(featom *atom, regionhandle region)
{
    ...
    fontPrintf(region->rect.x1 - width - CM_RUMarginRight,
               ...
               FEC_ListItemStandard, "%d", universe.curPlayerPtr->resourceUnits);
...
void cmTotalShipsDraw(featom *atom, regionhandle region)
{
    ...
    feStaticRectangleDraw(region);                          //draw regular rectangle as backdrop
    fontPrintf((region->rect.x0 + region->rect.x1 - width) / 2,
               (region->rect.y1 - region->rect.y0 - fontHeight(NULL)) / 2 + region->rect.y0,
               FEC_ListItemStandard, "%d", nShips);
    fontMakeCurrent(oldfont);
}
...
bool32 cmCanBuildShipType(Ship *factoryship,ShipType shiptype,bool32 checkResearch)
{
    ...
    race = cmFactoryBuildRace(factoryship);
    ...
    for (index = 0; shipsavail[index].nJobs != -1; index++)
    {
        if (shipsavail[index].itemtype==ITEM_SHIP)
        {
            if (shipsavail[index].type == shiptype)
            {
                ...
                if (cmCaptureBuildAllFactoryEnabled(factoryship) ||
                    rmCanBuildShip(factoryship->playerowner, shiptype))
                {
                    return TRUE;
                }
                else
...
sdword cmUpdateFactoryNoReset(Ship* shipfactory)
{
    ...
    race = cmFactoryBuildRace(shipfactory);
...
void cmUpdateShipsAvailable(void)
{
    ...
    {
        cmDockFactoryNormalizeSelection();
        if (cmDockSelectedFactory != NULL)
        {
            cmUpdateFactoryNoReset(cmDockSelectedFactory);
        }
        return;
    }
...
sdword cmUpdateFactory(Ship* shipfactory)
{
    ...
    for (index = 0; index < TOTAL_NUM_SHIPS; index++)
    {
        curshipsInProgress->progress[index].selected = FALSE;
    }
...
sdword cmConstructionBegin(regionhandle region, smemsize ID, udword event, udword data)
{
    ...
    if (mdManagerDockEnabled())
    {
        if (!cmDockFactoryVisible(bship))
        {
            if (cmDockFactoryVisible(cmDockSelectedFactory))
            {
                bship = cmDockSelectedFactory;
            }
            else if (cmDockFactoryVisible(universe.curPlayerPtr->PlayerMothership))
            {
                bship = universe.curPlayerPtr->PlayerMothership;
            }
            else
            {
                bship = cmDockFactoryAt(0);
            }
        }
        cmDockSelectedFactory = bship;
        cmDockFactoryNormalizeSelection();
    }
    else if ((bship == NULL) ||
        (((bship->shiptype != Mothership) && (bship->shiptype != Carrier)) &&
         !cmCaptureBuildAllFactoryEnabled(bship)))
    {
        if (cmCurrentSelect==0)
        {
            bship = universe.curPlayerPtr->PlayerMothership;
    ...
    // Docked managers are overlays; the tutorial retains the original
    // full-screen presentation because its scripted pointers use FIB coords.
    if (!mdManagerDockEnabled())
    {
    ...
    mdPrepareManagerScreen(feScreenFind(CM_ConstructionScreen),
                           HW_MANAGER_DOCK_BUILD);
    ...
    cmIoSaveState = mdManagerDockEnabled() ? FALSE : ioDisable();

    cmBaseRegion = feScreenStart(region, CM_ConstructionScreen);//add new regions as siblings of current one
    mdInstallManagerToggleKeys(cmBaseRegion);
    cmDockFactoryInstall(cmBaseRegion);
...
//    sdword index;
    ...
    /* Do not let stale Build Manager state or a build hotkey bypass the
       Gameplay option.  Existing queued jobs continue elsewhere; this only
       rejects new foreign-factory submissions while the option is OFF. */
    if (factory != NULL && cmCapturedForeignFactory(factory->ship) &&
        !cmCaptureBuildAllFactoryEnabled(factory->ship))
    {
        return;
    }
...
#endif
    dbgAssertOrIgnore(info->shiptype < TOTAL_NUM_SHIPS);                 //make sure we can add a job
...
void cmFillInCarrierXs(void)
{
    ...
    for (insert=0,index=0;index<cmNumCarriers;index++)
    {
        if (cmCarriers[index].owner == universe.curPlayerPtr &&
            (cmCarriers[index].ship->shiptype == Carrier ||
             cmCaptureBuildAllFactoryEnabled(cmCarriers[index].ship)))
        {
...
bool32 cmDeterministicSanityCheck(cmDetermProgress_t* dprog)
{
    ...
    //check race.  Multiplayer remains exactly owner-race only.  In
    //single-player CAPTURE / BUILD ALL may legitimately create a captured
    //factory's native race, but only from that native production roster.
    if (dprog->shipRace != player->race)
    {
        if (!singlePlayerGame ||
            dprog->creator == NULL ||
            dprog->creator->playerowner != player ||
            !cmCapturedForeignFactory(dprog->creator) ||
            cmFactoryBuildRace(dprog->creator) != dprog->shipRace ||
            !cmCanBuildShipType(dprog->creator, dprog->shipType, FALSE))
        {
            rval = FALSE;
        }
    }

    //check type.  Non-standard P1/P2 types are only valid when the same
    //captured production hull is explicitly allowed to build that native type.
    if (dprog->shipType < STD_FIRST_SHIP ||
        dprog->shipType > STD_LAST_SHIP)
    {
        if (dprog->creator == NULL ||
            !cmCapturedForeignFactory(dprog->creator) ||
            !cmCanBuildShipType(dprog->creator, dprog->shipType, FALSE))
        {
            rval = FALSE;
...
shipsinprogress *Load_shipsinprogress(void)
{
    ...
    shipsinprogress *sinprogress;
    sdword i;
    sdword progressCount;

    chunk = LoadNextChunk();
    VerifyChunkNoSize(chunk,BASIC_STRUCTURE|SAVE_SHIPSINPROGRESS);
    dbgAssertOrIgnore(chunk->contentsSize == sizeof(shipsinprogress) ||
                      chunk->contentsSize == sizeof(cmLegacyShipsInProgress));

    sinprogress = memAlloc(sizeof(shipsinprogress),"sinprogress",NonVolatile);
    memset(sinprogress,0,sizeof(shipsinprogress));
    if (chunk->contentsSize == sizeof(cmLegacyShipsInProgress))
    {
        memcpy(sinprogress,chunkContents(chunk),sizeof(cmLegacyShipsInProgress));
        progressCount = TOTAL_STD_SHIPS;
    }
    else
    {
        memcpy(sinprogress,chunkContents(chunk),sizeof(shipsinprogress));
        progressCount = TOTAL_NUM_SHIPS;
    }
...
void LoadConsMgr(void)
{
    ...
    /* Older saves predate captured non-Carrier production entries in
       cmCarriers.  Reconstruct those selector records from the factory list. */
    {
        Node *node = listofShipsInProgress.head;
        while (node != NULL)
        {
            shipsinprogress *loadedFactory = (shipsinprogress *)listGetStructOfNode(node);
            Ship *loadedShip = loadedFactory->ship;
            if (cmIsForeignProductionShip(loadedShip))
            {
                sdword found = FALSE;
                for (i = 0; i < cmNumCarriers; i++)
                {
                    if (cmCarriers[i].ship == loadedShip)
                    {
                        found = TRUE;
                        break;
                    }
                ...
                {
                    cmNumCarriers++;
                    cmCarriers = memRealloc(cmCarriers,
                        sizeof(CarrierInfo) * cmNumCarriers,
                        "cmCarriers", NonVolatile);
                    cmCarriers[cmNumCarriers - 1].ship = loadedShip;
                    cmCarriers[cmNumCarriers - 1].owner = loadedShip->playerowner;
                }
            }
            node = node->next;
        }
