...
typedef enum
{
    BIG_TOC_LAYOUT_INVALID = 0,
    BIG_TOC_LAYOUT_CLASSIC,
    BIG_TOC_LAYOUT_REMASTERED
}
bigTOCLayout;

static void bigTOCRemasteredEntryConvert(bigTOCFileEntry *destination,
                                         const bigTOCFileEntryRemaster *source)
{
    destination->nameCRC1        = source->nameCRC1;
    destination->nameCRC2        = source->nameCRC2;
    destination->nameLength      = source->nameLength;
    destination->storedLength    = source->storedLength;
    destination->realLength      = source->realLength;
    destination->offset          = source->offset;
    destination->timeStamp       = source->timeStamp;
    destination->compressionType = source->compressionType;
}

static void bigTOCEntryFixEndian(bigTOCFileEntry *entry)
{
#if FIX_ENDIAN
    entry->nameCRC1     = FIX_ENDIAN_INT_32(entry->nameCRC1);
    entry->nameCRC2     = FIX_ENDIAN_INT_32(entry->nameCRC2);
    entry->nameLength   = FIX_ENDIAN_INT_16(entry->nameLength);
    entry->storedLength = FIX_ENDIAN_INT_32(entry->storedLength);
    entry->realLength   = FIX_ENDIAN_INT_32(entry->realLength);
    entry->offset       = FIX_ENDIAN_INT_32(entry->offset);
    entry->timeStamp    = FIX_ENDIAN_INT_32(entry->timeStamp);
#else
    (void)entry;
#endif
}

static bool32 bigTOCEntryLooksValid(const bigTOCFileEntry *entry,
                                    long tableEnd, long fileSize)
{
    uqword dataStart;
    uqword dataEnd;

    if (entry->nameLength == 0 || entry->nameLength > BF_MAX_FILENAME_LENGTH ||
        (entry->compressionType != 0 && entry->compressionType != 1) ||
        entry->offset < (udword)tableEnd)
    {
        return FALSE;
    }

    if (!entry->compressionType && entry->storedLength != entry->realLength)
    {
        return FALSE;
    }

    dataStart = (uqword)entry->offset + entry->nameLength + 1;
    dataEnd = dataStart + entry->storedLength;
    return dataStart <= (uqword)fileSize && dataEnd <= (uqword)fileSize;
}

/*
 * Gearbox's Homeworld Remastered Classic archive added one udword to every
 * TOC entry without changing the RBF version.  Older ports guessed the format
 * from one exact file count (13887), which breaks as soon as another language
 * or patched archive contains a different number of files.  Score samples
 * from both layouts instead and accept the one whose decoded offsets, lengths,
 * and compression fields describe the actual archive.
 */
static int bigTOCLayoutScore(FILE *fp, long tableStart, long fileSize,
                             int numFiles, bigTOCLayout layout)
{
    const int maxSamples = 32;
    size_t entrySize = layout == BIG_TOC_LAYOUT_REMASTERED
                     ? sizeof(bigTOCFileEntryRemaster)
                     : sizeof(bigTOCFileEntry);
    int sampleCount = numFiles < maxSamples ? numFiles : maxSamples;
    long tableEnd;
    int sample;
    int score = 0;

    if (numFiles <= 0 || tableStart < 0 || fileSize < tableStart ||
        (uqword)numFiles > (uqword)(fileSize - tableStart) / entrySize)
    {
        return -1;
    }

    tableEnd = tableStart + (long)((uqword)numFiles * entrySize);
    for (sample = 0; sample < sampleCount; ++sample)
    {
        int fileIndex = sampleCount == 1
                      ? 0
                      : (int)(((uqword)sample * (numFiles - 1)) / (sampleCount - 1));
        bigTOCFileEntry entry;

        if (fseek(fp, tableStart + (long)((uqword)fileIndex * entrySize), SEEK_SET) != 0)
        {
            return -1;
        }

        if (layout == BIG_TOC_LAYOUT_REMASTERED)
        {
            bigTOCFileEntryRemaster remasteredEntry;
            if (fread(&remasteredEntry, sizeof(remasteredEntry), 1, fp) != 1)
            {
                return -1;
            }
            bigTOCRemasteredEntryConvert(&entry, &remasteredEntry);
        }
        else if (fread(&entry, sizeof(entry), 1, fp) != 1)
        {
            return -1;
        }

        bigTOCEntryFixEndian(&entry);
        if (bigTOCEntryLooksValid(&entry, tableEnd, fileSize))
        {
            ++score;
        }
    }
...
static bigTOCLayout bigTOCLayoutDetect(FILE *fp, long tableStart,
                                       long fileSize, int numFiles)
{
    int classicScore = bigTOCLayoutScore(fp, tableStart, fileSize, numFiles,
                                         BIG_TOC_LAYOUT_CLASSIC);
    int remasteredScore = bigTOCLayoutScore(fp, tableStart, fileSize, numFiles,
                                            BIG_TOC_LAYOUT_REMASTERED);

    if (classicScore <= 0 && remasteredScore <= 0)
    {
        return BIG_TOC_LAYOUT_INVALID;
    }

    return remasteredScore > classicScore
         ? BIG_TOC_LAYOUT_REMASTERED
         : BIG_TOC_LAYOUT_CLASSIC;
}
...
static int bigTOCRead(FILE *fp, bigTOC *toc)
{
    ...
    toc->numFiles = 0;
    toc->flags = 0;
    toc->fileEntries = NULL;

    if (fread(&toc->numFiles, sizeof(toc->numFiles), 1, fp) != 1 ||
        fread(&toc->flags, sizeof(toc->flags), 1, fp) != 1)
    {
        return 0;
    }
    ...
    tableStart = ftell(fp);
    if (tableStart < 0 || fseek(fp, 0, SEEK_END) != 0)
    {
        return 0;
    }
    fileSize = ftell(fp);
    ...
    layout = bigTOCLayoutDetect(fp, tableStart, fileSize, toc->numFiles);
    if (layout == BIG_TOC_LAYOUT_INVALID || fseek(fp, tableStart, SEEK_SET) != 0)
    {
        return 0;
    }

    toc->fileEntries = malloc(sizeof(*toc->fileEntries) * toc->numFiles);
    if (toc->fileEntries == NULL)
    {
        return 0;
    }
    ...
    if (layout == BIG_TOC_LAYOUT_REMASTERED)
    {
        bigTOCFileEntryRemaster *remasteredEntries =
            malloc(sizeof(*remasteredEntries) * toc->numFiles);

        if (remasteredEntries == NULL ||
            fread(remasteredEntries, sizeof(*remasteredEntries), toc->numFiles, fp)
                != (size_t)toc->numFiles)
        {
            free(remasteredEntries);
        ...
        for (i = 0; i < toc->numFiles; ++i)
        {
            bigTOCRemasteredEntryConvert(&toc->fileEntries[i], &remasteredEntries[i]);
        }
        free(remasteredEntries);
    }
    else if (fread(toc->fileEntries, sizeof(*toc->fileEntries), toc->numFiles, fp)
             != (size_t)toc->numFiles)
    {
        free(toc->fileEntries);
        toc->fileEntries = NULL;
    ...
    for (i = 0; i < toc->numFiles; ++i)
    {
        bigTOCEntryFixEndian(&toc->fileEntries[i]);
    }
...
bool32 bigOpenAllBigFiles(void)
{
    ...
    for (bigfile_i = 0; bigfile_i < NUMBER_CONFIGURED_BIG_FILES; ++bigfile_i)
    {
        ...
        if (bigFilePrecedence[bigfile_i].filePtr == NULL)
        {
            ...
            {
                // Remastered Classic folds the 1.05 content into Homeworld.big.
                // A missing Update.big is therefore an expected configuration.
                dbgMessagef("Optional Update.big not present; using Homeworld.big data");
                continue;
            }
        }
        ...
        if (!bigHeaderVerify(bigFilePrecedence[bigfile_i].filePtr))
        {
            ...
            {
                dbgWarningf(DBG_Loc, "Ignoring incompatible optional .big file: %s", bigFilePath);
                fclose(bigFilePrecedence[bigfile_i].filePtr);
                bigFilePrecedence[bigfile_i].filePtr = NULL;
                continue;
            }
        }
        ...
        if (bigFilePrecedence[bigfile_i].filePtr != NULL)
        {
            if (!bigTOCRead(bigFilePrecedence[bigfile_i].filePtr,
                            &bigFilePrecedence[bigfile_i].tableOfContents))
            {
                if (bigFilePrecedence[bigfile_i].required)
                {
                    dbgFatalf(DBG_Loc, "Unable to read required .big file TOC: %s", bigFilePath);
                }

                dbgWarningf(DBG_Loc, "Ignoring unreadable optional .big file: %s", bigFilePath);
                fclose(bigFilePrecedence[bigfile_i].filePtr);
                bigFilePrecedence[bigfile_i].filePtr = NULL;
                continue;
            }
            ...
            bigFilePrecedence[bigfile_i].localFileRelativeAge
                = memAlloc(sizeof(*bigFilePrecedence[bigfile_i].localFileRelativeAge) *
                           bigFilePrecedence[bigfile_i].tableOfContents.numFiles,
                           "bigLocalFileRelativeAge", 0);                   // 0 => no memAlloc flags
                           
            memset(bigFilePrecedence[bigfile_i].localFileRelativeAge,
                0, sizeof(*bigFilePrecedence[bigfile_i].localFileRelativeAge) *
                   bigFilePrecedence[bigfile_i].tableOfContents.numFiles);  // 0 = LOCAL_FILE_DOES_NOT_EXIST
        }
    }
...
sdword bigArchiveCount(void)
{
    return (sdword)numOpenedBigFiles;
}

sdword bigArchiveFileCount(sdword archiveIndex)
{
    if (archiveIndex < 0 || archiveIndex >= (sdword)numOpenedBigFiles)
        return 0;
    return bigFilePrecedence[archiveIndex].tableOfContents.numFiles;
}

const char *bigArchiveName(sdword archiveIndex)
{
    if (archiveIndex < 0 || archiveIndex >= (sdword)numOpenedBigFiles)
        return NULL;
    return bigFilePrecedence[archiveIndex].bigFileName;
}

bool32 bigArchiveFileName(sdword archiveIndex, sdword fileIndex,
                          char *outName, sdword outNameBytes)
{
    bigFileConfiguration *config;
    bigTOCFileEntry *entry;
    char temp[BF_MAX_FILENAME_LENGTH + 1];
    if (outName == NULL || outNameBytes <= 0)
        return FALSE;
    outName[0] = 0;
    if (archiveIndex < 0 || archiveIndex >= (sdword)numOpenedBigFiles)
        return FALSE;
    config = &bigFilePrecedence[archiveIndex];
    if (config->filePtr == NULL || fileIndex < 0 ||
        fileIndex >= config->tableOfContents.numFiles)
        return FALSE;
    entry = &config->tableOfContents.fileEntries[fileIndex];
    if ((sdword)entry->nameLength >= (sdword)sizeof(temp))
        return FALSE;
    if (!bigFilenameRetrieve(&config->tableOfContents, fileIndex,
                             config->filePtr, temp))
        return FALSE;
    temp[sizeof(temp) - 1] = 0;
    strncpy(outName, temp, (size_t)outNameBytes - 1);
...
void bigCRC(udword *bigCRCArray, udword arraySize)
{
    udword  bigfile_i = 0,
            crcCount  = 1,
            arrayEntries = arraySize / sizeof(*bigCRCArray),
           *crcPtr    = bigCRCArray + 1;
