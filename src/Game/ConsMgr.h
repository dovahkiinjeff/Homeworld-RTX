...
typedef struct
{
    ...
    /* P1/P2 production tables were present in the original game but the
       queue stopped at STD_LAST_SHIP, making those entries unsafe if a
       non-player production hull ever became player-owned. */
    shipinprogress progress[TOTAL_NUM_SHIPS];
}
shipsinprogress;
...
/* CAPTURE / BUILD ALL helpers.  These are deliberately single-player and
   only recognize ships whose native static data already says they are
   production hulls. */
bool32 cmCapturedForeignFactory(const struct Ship *ship);
bool32 cmCaptureBuildAllFactoryEnabled(const struct Ship *ship);
ShipRace cmFactoryBuildRace(const struct Ship *ship);
