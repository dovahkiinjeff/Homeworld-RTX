...
static void smHideLegacyRegionTree(regionhandle reg);
static void smKeepShipFocusTargets(void);
static void smFocusMothershipClass(void);
...
void smViewportRender(featom *atom, regionhandle region)
{
    ...
    else if (smZoomingIn)
    {
        ...
        {                                                   //else rendering main game screen
            ...
            {                                               //if at transition point
                ...
                /* Do not render the Sensor viewport again on the renderer
                   hand-off frame.  The old behavior produced the bordered
                   inset rectangle visible during close transitions. */
                return;
            }
            else
    ...
    /* RTX-0046: Sensor Manager now participates in the same variable-cursor
       state as the normal tactical viewport.  Alt therefore advertises focus
       immediately and Ctrl / Ctrl+Shift advertise attack / force attack. */
...
void smCullAndFocusSelecting(void)
{
    ...
    nToFocusOn = ccFocusCullRadiusMean((FocusCommand *)&selSelecting,
                                       smFocusRadius * smFocusRadius, &centre);
    if (nToFocusOn == 0)
    {
        return;
    }

    /* RTX-0046: Homeworld 2 style Sensor focus.  F/Alt-click/Alt-bandbox
       retarget the Sensor camera without leaving Sensors.  Retain the same
       focus set so Space/Escape can hand it to the normal camera on exit. */
    if (smFocusCommand != NULL)
    {
        memFree(smFocusCommand);
        smFocusCommand = NULL;
    ...
    smFocusCommand = memAlloc(ccFocusSize(selSelecting.numTargets),
                              "SensorsMultiFocus", 0);
    ...
    /* Preserve Sensor zoom/distance: this is a map recenter, not the old
       "focus and leave Sensors" zoom transition. */
    smCameraLookatPoint = centre;
    smCameraLookVelocity.x = 0.0f;
    smCameraLookVelocity.y = 0.0f;
    smCameraLookVelocity.z = 0.0f;
    selRectNone();
}
    ...
    switch (event)
    {
        ...
        case RPE_PressLeft:
            ...
            if (smHoldRight == smNULL && (!smFleetIntel))
            {
                ...
                smHoldLeft = smSelectHold;
                smSelectionRect.x0 = smSelectionRect.x1 =
                    smSelectionRect.y0 = smSelectionRect.y1 = 0;
            }
            break;
        case RPE_ReleaseLeft:
            ...
            {
                vector destination;
                bool32 wasHyperspace = MP_HyperSpaceFlag;

                dbgAssertOrIgnore(!smFleetIntel);
                MakeShipsMobile((SelectCommand *)&selSelected);
                if (selSelected.numShips > 0)
                {
                    destination = (piePointSpecZ != 0.0f) ? pieHeightPoint : piePlanePoint;
                    if (!(isnan((double)destination.x) || isnan((double)destination.y) || isnan((double)destination.z)))
                    {
                        if (mrNeedProbeHack())
                        {
                        ...
                        /* The old Sensor chrome is hidden in normal Sensors, but
                           keep its button state coherent for scripts/Fleet Intel. */
                        if (wasHyperspace)
                        {
            ...
            {
                dragged = (ABS(mouseCursorX() - mrOldMouseX) >= selClickBoxWidth ||
                           ABS(mouseCursorY() - mrOldMouseY) >= selClickBoxHeight);
                guardMode = (keyIsHit(GKEY) ||
                             (keyIsHit(CONTROLKEY) &&
                              (keyIsHit(ALTKEY) || keyIsHit(RALTKEY))));

                if (dragged)
                {
                    mrSelectRectBuild(&smSelectionRect, mrOldMouseX, mrOldMouseY);

                    if (guardMode)
                    {
                        if (!mrDisabled)
                        {
                            selRectDragAnybodyAnywhere(&smCamera, &smSelectionRect);
                            if (selSelecting.numTargets > 0 && selSelected.numShips > 0)
                            {
                    ...
                    {
                        /* HW2-style focus: center the Sensor view and remain in it. */
                        selRectDragAnybodyAnywhere(&smCamera, &smSelectionRect);
                        smKeepShipFocusTargets();
                    ...
                    {
                        /* Same Ctrl/Ctrl+Shift attack semantics as main gameplay,
                           but select targets against the Sensor camera/list. */
                        if (!mrDisabled)
                        {
                            ...
                            selRectDragFunction(universe.SpaceObjList.head, &smCamera, &smSelectionRect,
                    ...
                    else if (!mrDisabled && piePointSpecMode != PSM_Waiting &&
                             ((!(tutorial==TUTORIAL_ONLY)) || tutEnable.bBandSelect))
                    {
                        /* Plain/Shift band-select now really selects while Sensors
                           stays open; it no longer invokes the old focus/exit path. */
                        if (keyIsHit(SHIFTKEY))
                        {
                            selRectDragAddFunction(universe.SpaceObjList.head, &smCamera, &smSelectionRect);
                        }
                        else
                        {
                            selRectDragFunction(universe.SpaceObjList.head, &smCamera, &smSelectionRect,
                            ...
                            if ((selSelected.numShips > 0) && (!playPackets) && (!universePause && opPauseOrders))
                            {
                                if (universe.curPlayerPtr->PlayerMothership != NULL &&
                                    selShipInSelection(selSelected.ShipPtr, selSelected.numShips,
                                                       universe.curPlayerPtr->PlayerMothership) &&
                ...
                else if (smHoldRight == smNULL)
                {
                    clickedOn = selSelectionClick(universe.SpaceObjList.head, &smCamera,
                                                   mouseCursorX(), mouseCursorY(), TRUE, TRUE);
                    if (clickedOn != NULL)
                    {
                        if ((keyIsHit(ALTKEY) || keyIsHit(RALTKEY)) && !guardMode)
                        {
                            if (clickedOn->objtype == OBJ_ShipType)
                            {
                                selSelecting.TargetPtr[0] = (SpaceObjRotImpTarg *)clickedOn;
        ...
        case RPE_PressRight:
            ...
            mrMouseHasMoved = 0;
            region->rect.x0 = region->rect.y0 = 0;
        ...
        case RPE_ReleaseRight:
            ...
            {
                mousePositionSet(mrOldMouseX, mrOldMouseY);
                mouseCursorShow();
                smHoldRight = smNULL;
                if (!mrDisabled && mrMouseHasMoved <= MR_MouseMovementClickLimit)
                {
                    /* mrRightClickMenu's modern path is selection-driven.  If
                       nothing is selected, pick the Sensor-camera ship first so
                       the context menu behaves like normal play, not the stale
                       hidden main-camera RenderList. */
                    if (selSelected.numShips == 0)
                    {
                        clickedOn = selSelectionClick(universe.SpaceObjList.head, &smCamera,
                                                       mouseCursorX(), mouseCursorY(), FALSE, FALSE);
                        if (clickedOn != NULL && clickedOn->objtype == OBJ_ShipType &&
                            clickedOn->playerowner == universe.curPlayerPtr &&
                            (clickedOn->flags & SOF_Selectable))
                        {
                            selSelectionSetSingleShip(clickedOn);
                            ioUpdateShipTotals();
                        }
                    }
                    mrRightClickMenu();
        ...
        case RPE_WheelDown:
            ...
            break;
        case RPE_KeyDown:
            /* Match normal gameplay's remapping before handling Sensor-specific
               camera/selection commands.  Forwarded commands still receive the
               original physical key so mrKeyPress performs its normal mapping. */
            commandID = kbCheckBindings(ID);
            /* Space/Escape are also hard exits while Sensors is active.  Keep
               the physical Space behavior even if the Sensors binding itself
               has been remapped, while still honoring a remapped Sensors key. */
            if (ID == SPACEKEY || ID == ESCKEY)
            {
                commandID = ID;
            }
            switch (commandID)
            {
                ...
                case MKEY:
                    if (!smFleetIntel)
                    {
                        ...
                        {
                            smFocusMothershipClass();
                        }
                        else if (!keyIsHit(SHIFTKEY) && !keyIsHit(CONTROLKEY))
                        {
                ...
                case MMOUSE_DOUBLE:
                case FKEY:
                    if (!smFleetIntel && selSelected.numShips > 0 &&
                        !(ID == MMOUSE_DOUBLE && (keyIsHit(ALTKEY) || keyIsHit(RALTKEY))))
                    {
                    ...
                    break;
                case ENTERKEY:
                    if (!smFleetIntel && lastshiptospeak != NULL)
                    {
                        if (lastshiptospeak == (Ship *)-1)
                        {
                            if (lastgrouptospeak >= 0 && lastgrouptospeak < SEL_NumberHotKeyGroups &&
                                selHotKeyGroup[lastgrouptospeak].numShips != 0)
                            {
                        ...
                        else if (!bitTest(lastshiptospeak->flags, SOF_Dead))
                        {
                            selSelecting.TargetPtr[0] = (SpaceObjRotImpTarg *)lastshiptospeak;
                            selSelecting.numTargets = 1;
                            smCullAndFocusSelecting();
                            soundEvent(NULL, UI_Click);
                            tutGameMessage("KB_FocusLast");
                ...
                case ARRUP:
                case ARRDOWN:
                    /* Sensor render polls these continuously for map panning. */
                    break;
#if SM_TOGGLE_SENSOR_LEVEL
                case LKEY:
                ...
                case NINEKEY:
                    if (!smFleetIntel &&
                        ((keyIsHit(ALTKEY) || keyIsHit(RALTKEY)) ||
                         (commandID == mrLastKeyPressed && universe.totaltimeelapsed <= mrLastKeyTime + mrNumberDoublePressTime)))
                    {
                        if (selHotKeyGroup[NUMKEYNUM(commandID)].numShips != 0)
                        {
                            selSelectHotKeyGroup(&selHotKeyGroup[NUMKEYNUM(commandID)]);
                            selHotKeyNumbersSet(NUMKEYNUM(commandID));
                            ioUpdateShipTotals();
                            selSelectionCopy((MaxAnySelection *)&selSelecting,(MaxAnySelection *)&selSelected);
                            ...
                            smCullAndFocusSelecting();
                            soundEvent(NULL, UI_Click);
                            speechEvent(selHotKeyGroup[NUMKEYNUM(commandID)].ShipPtr[0], COMM_AssGrp_Select, NUMKEYNUM(commandID));
                        }
                    }
                    else
                    {
                        mrRegionProcess(region, ID, event, data);
                    }
                    break;
                case EKEY:
                    if (!smFleetIntel && !keyIsHit(SHIFTKEY) && !keyIsHit(CONTROLKEY) &&
                        !keyIsHit(ALTKEY) && !keyIsHit(RALTKEY))
                    {
                        rectangle fullScreen;
                        fullScreen.x0 = 0;
                        fullScreen.y0 = 0;
                        fullScreen.x1 = MAIN_WindowWidth;
                        fullScreen.y1 = MAIN_WindowHeight;
                        selSelectionCopy((MaxAnySelection *)&tempSelection,(MaxAnySelection *)&selSelected);
                        selRectDragFunction(universe.SpaceObjList.head, &smCamera, &fullScreen,
                                            (SpaceObjRotImpTarg **)selSelected.ShipPtr, &selSelected.numShips,
                                            TRUE, FALSE, FALSE);
                        if (tempSelection.numShips != 0 &&
                            selSelectionCompare((MaxAnySelection *)&selSelected,
                                                (MaxAnySelection *)&tempSelection) == 0)
                        {
                    ...
                    break;
                default:
                    /* Everything not inherently camera-specific goes through the
                       same keybinding/command machinery as normal gameplay. */
                    mrRegionProcess(region, ID, event, data);
                    break;
            }
            mrLastKeyPressed = commandID;
...
//    dbgAssertOrIgnore(!smFleetIntel);
    ...
    /* If Sensor focus was changed with Alt/F, preserve that target through
       the existing close transition.  The hand-off copies it into the normal
       camera stack, then frees our temporary Sensor copy. */
    smFocus = (smFocusCommand != NULL && smFocusCommand->numShips > 0);
...
/* RTX-0046: ordinary Sensors reuses the gameplay HUD instead of the authored
   Sensor FE chrome.  Disable both drawing and input recursively so a child
   atom cannot flash through during the renderer hand-off. */
static void smHideLegacyRegionTree(regionhandle reg)
{
    regionhandle child;
    ...
    {
        smHideLegacyRegionTree(child);
    }
    regFilterSet(reg, 0);
    regDrawFunctionSet(reg, NULL);
}

/* RTX-0046: Sensor Alt-focus is a ship/group camera command, not a generic
   object targeting command.  The legacy Sensor drag can return resources and
   derelicts too, while the camera/slave helpers below operate on Ship pointers.
   Keep that contract explicit before casting the target array. */
static void smKeepShipFocusTargets(void)
{
    sdword readIndex;
    sdword writeIndex = 0;

    for (readIndex = 0; readIndex < selSelecting.numTargets; readIndex++)
    {
        if (selSelecting.TargetPtr[readIndex] != NULL &&
            selSelecting.TargetPtr[readIndex]->objtype == OBJ_ShipType)
        {
            selSelecting.TargetPtr[writeIndex++] = selSelecting.TargetPtr[readIndex];
        }
    }
    selSelecting.numTargets = writeIndex;
}

/* Keep Home / Alt-M semantics consistent with normal play, including captured
   carrier/mothership-class ships such as P1Mothership. */
static void smFocusMothershipClass(void)
{
    bool32 focusedOnCurrent = FALSE;

    if (mrMothershipPtr != NULL && smFocusCommand != NULL &&
        smFocusCommand->numShips == 1 &&
        smFocusCommand->ShipPtr[0] == mrMothershipPtr)
    {
        focusedOnCurrent = TRUE;
    }

    if (mrMothershipPtr == NULL || focusedOnCurrent)
    {
    ...
    {
        selSelecting.TargetPtr[0] = (SpaceObjRotImpTarg *)mrMothershipPtr;
        selSelecting.numTargets = 1;
        smCullAndFocusSelecting();
        soundEvent(NULL, UI_Click);
...
void smSensorsBegin(char *name, featom *atom)
{
    ...
    fescreen *screen;
    featom *pAtom;
    sdword smViewExpandLeft = 0;
    sdword smViewExpandRight = 0;
    sdword smViewExpandTop = 0;
    sdword smViewExpandBottom = 0;
    char *screenName;
    ...
    /* RTX-0046: ordinary Sensors is a gameplay viewport, not a modal UI
       replacement.  Fleet Intel retains the legacy modal teardown. */
    if (smFleetIntel)
    {
    ...
    {
        /* Keep the normal selection/info overlay alive in Sensors. */
        ioSaveState = FALSE;
        bitClear(tbDisable, TBDISABLE_SENSORS_USE);
    }

    /* Defensive cleanup in case a previous Sensor session was interrupted
       before its pending focus command reached the close transition. */
    if (smFocusCommand != NULL)
    {
        memFree(smFocusCommand);
        smFocusCommand = NULL;
...
//                         colUbyteToReal(colBlue(smBackgroundColor)), 1.0f);
    ...
    */
    if (smFleetIntel)
    {
        /* Fleet Intel keeps the authored letterbox/chrome and its legacy
    ...
    {
        /* Normal Sensor Manager: the old FE side/top/bottom chrome is the
           element that visibly pops during the transition.  Keep only the
           Sensor world viewport and leave the persistent gameplay HUD above it. */
        reg = baseRegion->child;
        smScrollListTop = smScrollListLeft = smScrollListRight = smScrollListBottom = NULL;
        smScrollCountLeft = smScrollCountTop = smScrollCountRight = smScrollCountBottom = 0;
        smScrollDistLeft = smScrollDistTop = smScrollDistRight = smScrollDistBottom = 0;
        smScrollLeft = smScrollTop = smScrollRight = smScrollBottom = 0;
        smViewExpandLeft = smViewExpandRight = smViewExpandTop = smViewExpandBottom = 0;

        for (index = 1; index < screen->nAtoms; index++, reg = reg->next)
        {
            regVerify(reg);
            if (reg != smViewportRegion)
            {
                smHideLegacyRegionTree(reg);
            }
        }

        smViewRectangle.x0 = 0;
        smViewRectangle.y0 = 0;
        smViewRectangle.x1 = MAIN_WindowWidth;
        smViewRectangle.y1 = MAIN_WindowHeight;
        smViewportRegion->rect = smViewRectangle;
    }
...
//    bobListCreate(&smBlobProperties, &smBlobList, universe.curPlayerIndex);
    ...
    /* Register the complete normal gameplay key surface.  Sensor-specific
       camera/focus/exit keys are intercepted by smViewportProcess; all other
       keys are forwarded to mrRegionProcess/mrKeyPress. */
    mrRegisterGameplayKeyChildren(smViewportRegion, (regionfunction)smViewportProcess);
    mouseCursorShow();
    ...
    /* Sensor FE was created after the gameplay HUD, so reassert the normal
       gameplay layers once the viewport exists.  Fleet Intel intentionally
       remains modal. */
    if (!smFleetIntel)
    {
        tbSensorsHook();
        ioBringToFront();
        modernUICommandBarBringToFront();
    }
}
