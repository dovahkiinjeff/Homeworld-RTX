...
/*-----------------------------------------------------------------------------
    ...
    Return      : void
----------------------------------------------------------------------------*/
void mrRegisterGameplayKeyChildren(regionhandle parent, regionfunction handler)
{
    sdword index;

    for (index = 0; mrKeyFunction[index].nKeys != 0; index++)
    {
        regKeyChildAlloc(parent, mrKeyFunction[index].key[0], mrKeyFunction[index].filter,
                         handler, mrKeyFunction[index].nKeys,
...
void mrStartup(void)
{
    ...
    mrRegisterGameplayKeyChildren(ghMainRegion, (regionfunction)mrRegionProcess);
...
void mrBuildShips(char *string, featom *atom)
{
    modernUICloseLightingEditor();
    modernUICloseShipDossier();
    if (mdManagerDockEnabled() && cmActive)
    {
        cmCloseIfOpen();
        return;
    }
    ...
    {
        mdCloseOtherManagers(HW_MANAGER_DOCK_BUILD);
    }
...
void mrInfo(char *string, featom *atom)
{
    ...
    primaryType = selSelected.ShipPtr[0]->shiptype;
    for (index = 1; index < selSelected.numShips; ++index)
    {
        if (selSelected.ShipPtr[index]->shiptype != primaryType)
        {
            mixedSelection = TRUE;
            break;
        }
    }
    modernUIShowShipDossier(selSelected.ShipPtr[0],
                            selSelected.numShips, mixedSelection);
}
...
void mrLaunch(char *string, featom *atom)
{
    modernUICloseLightingEditor();
    modernUICloseShipDossier();
    if (mdManagerDockEnabled() && lmActive)
    {
        lmCloseIfOpen();
        return;
    }
    ...
    if (mdManagerDockEnabled())
    {
        mdCloseOtherManagers(HW_MANAGER_DOCK_LAUNCH);
    }
...
void mrResearch(char *string, featom *atom)
{
    modernUICloseLightingEditor();
    modernUICloseShipDossier();
    if (mdManagerDockEnabled() && rmGUIActive)
    {
        rmCloseIfOpen();
        return;
    }
    ...
    if (mdManagerDockEnabled())
    {
        mdCloseOtherManagers(HW_MANAGER_DOCK_RESEARCH);
    }
...
void mrKeyPress(sdword ID)
{
    ...
    /* Shift+F11 opens the realtime map editor before gameplay key translation,
       so the legacy F11 formation binding cannot consume the editor chord. */
    if (ID == F11KEY &&
        (keyIsHit(LSHIFTKEY) || keyIsHit(RSHIFTKEY)) &&
        !(keyIsHit(LCONTROLKEY) || keyIsHit(RCONTROLKEY)))
    {
        modernMapEditorToggle();
        return;
    }
    /* Shift+F12 is reserved for the live lighting editor.  The two authoring
       UIs are mutually exclusive so their full-screen input regions never
       fight each other.  Ctrl+Shift+F12 remains the renderer-reset chord. */
    if (ID == F12KEY &&
        (keyIsHit(LSHIFTKEY) || keyIsHit(RSHIFTKEY)) &&
        !(keyIsHit(LCONTROLKEY) || keyIsHit(RCONTROLKEY)))
    {
        if (modernMapEditorActive()) modernMapEditorCloseForToolSwitch();
        modernUIToggleLightingEditor();
        return;
    }
    if (modernMapEditorActive())
    {
        /* The editor owns gameplay keyboard input while open. */
        modernMapEditorHandleKey(ID);
        return;
    }
...
static void mrModernizeMenuWidth(fescreen *screen)
{
    featom *base;
    sdword desired;
    sdword index;
    sdword padding = uiScaleSize(14);
    sdword minimum = uiScaleSize(150);
    sdword maximum = min(MAIN_WindowWidth * 45 / 100, uiScaleSize(420));

    if (screen == NULL || screen->nAtoms <= 0) return;
    base = &screen->atoms[0];
    desired = base->width;

    /* The original context-menu FIBs are narrow enough that the modern font
       clip rectangle turns FORMATIONS/DELTA/WALL/SPHERE into fragments.
       Measure the actual localized strings and widen the live menu geometry
       instead of abbreviating commands. */
    for (index = 0; index < screen->nAtoms; ++index)
    {
        featom *atom = &screen->atoms[index];
        char *text = mrMenuLocalizedText(atom);
        if (text != NULL && *text != 0)
        {
            fonthandle old = fontMakeCurrent((fonthandle)atom->attribs);
            sdword textWidth = fontWidth(text);
            fontMakeCurrent(old);
            desired = max(desired,
                          atom->x - base->x + textWidth + padding * 2);
        }
    }
    ...
    if (base->width < desired)
        base->width = (sword)desired;
    for (index = 1; index < screen->nAtoms; ++index)
    {
        featom *atom = &screen->atoms[index];
        sdword rightWidth = desired - (atom->x - base->x);
        if (rightWidth <= 0) continue;
        if (atom->type == FA_MenuItem || atom->type == FA_Divider ||
            atom->type == FA_DecorativeRegion ||
            atom->type == FA_OpaqueDecorativeRegion)
        {
            atom->width = (sword)rightWidth;
        }
        else if (atom->type == FA_StaticText &&
                 atom->width < rightWidth - padding)
        {
            atom->width = (sword)(rightWidth - padding);
        }
    }
}


/* Code-native HUD entry points for the existing formation/tactics menus.  The
   menu callbacks remain the original Homeworld command paths; only the launch
   location moves into the new persistent command strip. */
void mrOpenFormationSubmenuAt(sdword x, sdword y)
{
    fescreen *screen;
    featom *atom;
    ...
    if (selSelected.numShips < MIN_SHIPS_IN_FORMATION || ghMainRegion == NULL)
    {
        soundEvent(NULL, UI_ClickCancel);
        return;
    }

    screen = feScreenFind("Group_Formations_submenu");
    if (screen == NULL)
    {
        return;
    }
    mrModernizeMenuWidth(screen);
    while (feMenuLevel > 0)
    {
        feMenuDisappear(NULL, NULL);
    }
    y -= screen->atoms[0].height;
    if (y < 4) y = 4;
    for (index = 0; index < screen->nAtoms; ++index)
    {
        bitClear(screen->atoms[index].status, FAS_Checked);
    }
    formation = clSelectionAlreadyInFormation(&universe.mainCommandLayer,
                                               (SelectCommand *)&selSelected);
    if (formation != NO_FORMATION && formation <= CUSTOM_FORMATION)
    {
        atom = feAtomFindInScreen(screen, mrMenuItemByFormation[formation]);
        if (atom != NULL)
        {
            bitSet(atom->status, FAS_Checked);
        }
    }
    feMenuStart(ghMainRegion, screen, x, y);
    soundEvent(NULL, UI_Click);
}

void mrOpenTacticsSubmenuAt(sdword x, sdword y)
{
    fescreen *screen;
    featom *atom;
    ...
    if (selSelected.numShips <= 0 || ghMainRegion == NULL)
    {
        soundEvent(NULL, UI_ClickCancel);
        return;
    }
    for (index = 0; index < selSelected.numShips; ++index)
    {
        tacticsBits |= 1u << selSelected.ShipPtr[index]->tacticstype;
    }
    screen = feScreenFind("Group_Tactics_submenu");
    if (screen == NULL)
    {
        return;
    }
    ...
    {
        if (tacticsBits & (1u << index))
        {
            atom = feAtomFindInScreen(screen, mrMenuItemByTactic[index]);
            if (atom != NULL)
            {
                bitSet(atom->status, FAS_Checked);
            }
...
#define NUMBER_GAPS         12
    ...
    mrModernizeMenuWidth(staticScreen);
    ...
    *newScreen = *staticScreen;                             //copy the static screen to the temp screen
    ...
    /* Reserve one extra atom.  The shipped RightClickMenu FIB has no dossier
       entry, so the modern action must be synthesized after legacy filtering. */
    newScreen->atoms = memAlloc(sizeof(featom) * (staticScreen->nAtoms + 1), "tempMenuAtoms", 0);
    ...
    /* The original 1999 menu data does not contain CSM_Info.  Clone the
       geometry and interaction style of an existing functional item, append
       it after compaction, and give it the registered modern callback name.
       ModernUI renders the crisp SHIP DOSSIER caption itself. */
    if (bitTest(actionMask, MAM_Info))
    {
        featom *templateAtom = NULL;
        sdword bottom = 0;
        uword highestTabstop = 0;
        for (index = 0; index < newScreen->nAtoms; ++index)
        {
            featom *candidate = &newScreen->atoms[index];
            if (candidate->type == FA_MenuItem)
            {
                bottom = max(bottom, candidate->y + candidate->height);
                highestTabstop = max(highestTabstop, candidate->tabstop);
                if (templateAtom == NULL ||
                    (candidate->name != NULL &&
                     feNamesEqual(candidate->name, "CSM_Move")))
                {
                    templateAtom = candidate;
                }
            }
        }
        if (templateAtom != NULL)
        {
            featom dossierAtom = *templateAtom;
            dossierAtom.name = "CSM_Info";
            dossierAtom.type = FA_MenuItem;
            dossierAtom.flags &= ~(FAF_Link | FAF_Hidden | FAF_Disabled);
            dossierAtom.flags |= FAF_Function;
            dossierAtom.status &= ~FAS_Checked;
            dossierAtom.y = (sword)bottom;
            dossierAtom.loadedY = dossierAtom.y;
            dossierAtom.tabstop = highestTabstop + 1;
            dossierAtom.region = NULL;
            newScreen->atoms[newScreen->nAtoms++] = dossierAtom;
            if (newScreen->atoms[0].height <
                dossierAtom.y + dossierAtom.height)
            {
                newScreen->atoms[0].height =
                    dossierAtom.y + dossierAtom.height;
            }
        }
    }
    mrModernizeMenuWidth(newScreen);
...
void mrRightClickMenu(void)
{
    ...
    /* The modern context menu describes the active selection, not the pixel
       under the pointer.  A short right-click therefore works anywhere in the
       tactical viewport whenever one or more ships are selected. */
    if (selSelected.numShips > 0)
    {
        for (index = 0; index < selSelected.numShips; ++index)
        {
            ship = selSelected.ShipPtr[index];
            actionMask |= mrMenuActionsByShipType[ship->shiptype];
            tacticsBits |= 1 << ship->tacticstype;
        }
        formation = clSelectionAlreadyInFormation(&universe.mainCommandLayer,
                                                   (SelectCommand *)&selSelected);
        if (tutorial)
...
gotselection:
    ...
    {
        if (cmCaptureBuildAllFactoryEnabled(selSelected.ShipPtr[0]))
        {
            cmConstructionBegin(ghMainRegion,(smemsize)selSelected.ShipPtr[0], 0, 0);
        }
        else switch (selSelected.ShipPtr[0]->shiptype)
        {
            case Carrier:
            case Mothership:
/*
...
//                }
            ...
            case ResearchShip:
                ...
                break;
            case SensorArray:
                /* RTX-0046: Sensor Manager now reuses normal click handling.
                   Do not recursively open another Sensor Manager when a
                   selected Sensor Array is clicked while already inside it. */
                if (!smSensorsActive)
                {
                    smSensorsBegin(NULL, NULL);
                }
...
udword mrRegionProcess(regionhandle reg, sdword ID, udword event, udword data)
{
    ...
    if (modernMapEditorActive() && modernMapEditorHandleWorldEvent(event, data))
    {
        return RPR_Redraw;
    }
    ...
    switch (event)
    {
        ...
        case RPE_HoldRight:
            ...
            break;
       case RPE_PressLeft:
            /* A right-drag camera release can occasionally land on a child
               HUD region instead of this world region.  Recover stale mouse
               capture before beginning a new selection drag so band-boxing
               never feels intermittent after camera movement. */
            if (mrHoldRight != mrNULL && !mouseRightButton())
            {
                if (mrHoldRight == mrCameraMotion)
                {
            ...
            if (mrHoldLeft != mrNULL && !mouseLeftButton())
            {
                mrHoldLeft = mrNULL;
            }
...
endReleaseButtonLogic:
    ...
    case RPE_ReleaseRight:
            ...
            {                                               //if in camera movement mode
                ...
                if (!modernMapEditorActive() && !mrDisabled &&
                    mrMouseHasMoved <= MR_MouseMovementClickLimit)
...
//        aivarValueSet(aivRenderMainScreen, mrRenderMainScreen);
    ...
    else if (gameIsRunning)
    {
        ...
        /* Shift+F11 owns its final FPS camera pose after the legacy controller.
           ccControl supplies mouse yaw/pitch, then the editor discards fleet
           refocus/orbit/zoom and reapplies its fixed-eye look + wheel flight. */
        modernMapEditorCameraUpdate();
    }
    else
    {
        mrCamera = &defaultCamera;
    }
    if (modernMapEditorWireframeActive())
    {
    ...
    /* Shift+F11 owns an explicit selected-object manipulator. Drawing it here
       makes the live world path authoritative; it no longer depends on sibling
       editor-region draw ordering. Outside the editor this is a no-op. */
    modernMapEditorDrawWorldGizmo();
