...
void fontCharacterCreate(sdword x, sdword y, sdword width, sdword height, ubyte *source, ubyte *dest, sdword sourceWidth, sdword sourceHeight, bool32 antialias)
{
    sdword line;
...
----------------------------------------------------------------------------*/
static bool32 glfontDisplayCharacter(fontheader* font, char ch, real32 x, real32 y, color c)
{
#define VERT(S,T,X,Y) \
    ...
    if (lastGLHandle != page->glhandle)
    {
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                        scale == 1.0f ? GL_NEAREST : GL_LINEAR);
    ...
    {
        VERT(sBegin, tEnd,
             x + (real32)fcharacter->offsetX * scale,
             y + (real32)fcharacter->offsetY * scale);
        VERT(sBegin, tBegin,
             x + (real32)fcharacter->offsetX * scale,
             y + (real32)(fcharacter->offsetY + fcharacter->height) * scale);
        VERT(sEnd, tEnd,
             x + (real32)(fcharacter->offsetX + fcharacter->width) * scale,
             y + (real32)fcharacter->offsetY * scale);
        VERT(sEnd, tBegin,
             x + (real32)(fcharacter->offsetX + fcharacter->width) * scale,
             y + (real32)(fcharacter->offsetY + fcharacter->height) * scale);
...
----------------------------------------------------------------------------*/
bool32 glfontDisplayString(fontheader* font, char* string, sdword x, sdword y, color c)
{
    char* charp;
    glfontheader* glfont;
    glfontcharacter* character;
    charheader* fcharacter;
    real32 sx, sy;
    bool32 texOn, blendOn, alphatestOn;
    bool32 rval;
    sdword brights[3];
    color bright, colour;
    ...
    for (charp = string; *charp != '\0'; charp++)
    {
        ...
        if (character->page == NULL)
        {
            ...
            sx += (real32)font->spacing * uiScaleGet();
            continue;
        }
        ...
        sx += (real32)(fcharacter->width + font->spacing) * uiScaleGet();
    }
...
sdword fontLoadAndConvertTo64Bit(char *fileName, void** loadAddress)
{
    ...
    memcpy((ubyte *)newrootptr + sizeof(fontfileheader),
           (ubyte *)oldrootptr + sizeof(fontfileheader_disk),
           oldlength - sizeof(fontfileheader_disk));
...
sdword fontPrintN(sdword x, sdword y, color c, char *string, sdword maxCharacters)
{
    ...
    if (fontCurrentShadowType != FS_NONE)
    {
        ...
        xOffset = uiScaleSize(xOffset);
        yOffset = uiScaleSize(yOffset);
        fontPrintN(x + xOffset, y + yOffset, fontCurrentShadowColor, string, maxCharacters);
        fontCurrentShadowType = saveType;
    }

    if (fontCurrentFont != NULL &&
        hwModernTextDraw((fontCurrent < FR_NumberFonts &&
                          frFontRegistry[fontCurrent].name != NULL) ?
                             frFontRegistry[fontCurrent].name : "regular",
                         uiScaleSize(fontCurrentFont->fullHeight),
                         x, y, c, string, maxCharacters))
    {
        return TRUE;
    }
...
sdword fontWidthN(char *string, sdword maxCharacters)
{
    ...
    if (fontCurrentFont != NULL)
    {
        sdword modernWidth = hwModernTextWidth(
            (fontCurrent < FR_NumberFonts &&
             frFontRegistry[fontCurrent].name != NULL) ?
                frFontRegistry[fontCurrent].name : "regular",
            uiScaleSize(fontCurrentFont->fullHeight), string, maxCharacters);
        if (modernWidth > 0 || string == NULL || string[0] == '\0')
            return modernWidth;
    }
...
{
    return(uiScaleSize(fontCurrentFont->fullHeight));
}
