...
lifheader* tr64LifAdjustLoad(lifheader_disk *oldHeader, sdword oldLength)
{
    ...
    memcpy((ubyte *)newHeader + sizeof(lifheader),
           (ubyte *)oldHeader + sizeof(lifheader_disk),
           oldLength - sizeof(lifheader_disk));
...
lifheader *trLIFFileLoad(char *fileName, udword flags)
{
    ...
    /* tr64LifAdjustLoad returns a complete adjusted copy. Keeping the packed
       source block leaked every texture on x64 and became especially costly
       when the DXR material atlas inspected authored LIF texels. */
    memFree(oldHeader);
#else
...
/* -------------------------------------------------------------------------
   ...
   authority for registry metadata and team-colour effect masks; only the
   uploaded image pixels are replaced.  This allows high-resolution loose
   textures without changing textures.ll dimensions or LIF metadata.

   FF_IgnoreBIG is intentional: this override is specifically for loose data
   files and must never turn a PNG stored in a BIG archive into an implicit
   replacement. */
static bool32 trModernLoosePngPath(char *pngName, size_t pngNameSize,
                                   const char *sourceName)
{
    char *extension;
    size_t length;

    if (pngName == NULL || pngNameSize == 0 || sourceName == NULL ||
        sourceName[0] == 0)
        return FALSE;
    ...
    strncpy(pngName, sourceName, pngNameSize - 1);
...
static bool32 trModernLoosePngExists(const char *sourceName)
{
    char pngName[PATH_MAX];
    return trModernLoosePngPath(pngName, sizeof(pngName), sourceName);
}

static bool32 trModernLoadLooseGrayPng(const char *name,
                                       sdword expectedWidth,
                                       sdword expectedHeight, ubyte **result)
{
    void *fileData = NULL;
    sdword fileSize;
...
static ubyte *trModernLoosePngEffectMap(const lifheader *lifFile,
                                        const char *pngName,
                                        sdword width, sdword height,
                                        sdword effectIndex)
{
    char maskName[PATH_MAX];
    ubyte *explicitMask = NULL;
    ubyte *result;
    const ubyte *paletteEffect;
    sdword sourceWidth, sourceHeight;
    sdword x, y;
    char *extension;

    sourceWidth = lifFile->width;
    sourceHeight = lifFile->height;
    if (sourceWidth <= 0 || sourceHeight <= 0) return NULL;

    strncpy(maskName, pngName, sizeof(maskName) - 1);
    maskName[sizeof(maskName) - 1] = 0;
    extension = strrchr(maskName, '.');
    if (extension != NULL)
    {
        snprintf(extension,
    ...
    result = (ubyte *)memAlloc((memsize)width * height,
                                "Modern loose PNG scaled team mask", 0);
    paletteEffect = effectIndex == 0 ? lifFile->teamEffect0 : lifFile->teamEffect1;
    for (y = 0; y < height; ++y)
    {
        sdword sourceY = y * sourceHeight / height;
        for (x = 0; x < width; ++x)
        {
            sdword sourceX = x * sourceWidth / width;
            sdword sourceIndex = sourceY * sourceWidth + sourceX;
            ubyte value;
            if (explicitMask != NULL)
            {
                value = explicitMask[sourceIndex];
            }
            else if (bitTest(lifFile->flags, TRF_Paletted))
            {
                value = paletteEffect[lifFile->data[sourceIndex]];
            }
            else
            {
                value = paletteEffect[sourceIndex];
            }
            result[y * width + x] = value;
        }
    }
    if (explicitMask != NULL) memFree(explicitMask);
    return result;
}
                                      ...
                                      const char *lifName)
{
    ...
    if (reg == NULL || lifFile == NULL || lifName == NULL ||
        !trModernLoosePngPath(pngName, sizeof(pngName), lifName))
        return FALSE;
    ...
    /* The loose override is true-colour.  Preserve the original LIF's team
       colour flags and effect maps, but let the PNG control image resolution
       and alpha. */
    ...
    if (colorInfo != NULL)
    {
        reg->palettes = memAlloc(sizeof(udword) * reg->nPalettes +
                                 ...
                                 "Modern loose PNG RGB handles", NonVolatile);
        memcpy((ubyte *)reg->palettes + sizeof(udword) * reg->nPalettes,
               colorInfo, sizeof(trcolorinfo) * reg->nPalettes);
        colored = (color *)memAlloc((memsize)sizeof(color) * width * height,
                                    "Modern loose PNG colored image", 0);
        ...
        for (count = 0; count < reg->nPalettes; ++count)
        {
            if (!trUnusedInfo(&colorInfo[count]))
            {
                ...
                {
                    trBufferColorRGB(colored, (color *)pixels, effect0, effect1,
                                     colorInfo[count].base, colorInfo[count].detail,
                                     width * height, reg->flags,
                                     scalar0[count], scalar1[count]);
                    reg->handle = trRGBTextureCreate(colored, width, height,
                                                     useAlpha);
                }
                else
                {
                    reg->handle = trRGBTextureCreate((color *)pixels, width,
                                                     height, useAlpha);
                }
                if (reg->nPalettes > 1)
                {
    ...
    {
        reg->palettes = NULL;
        reg->handle = trRGBTextureCreate((color *)pixels, width, height,
                                         useAlpha);
    }
...
void trMeshSortListLoad(trmeshsort *sortList)
{
    ...
    for (index = 0; index < sortList->nTextures; index++)
    {
        ...
        {                                                   //if texture needs loading
            ...
            strncpy(sourceLifName, fullName, sizeof(sourceLifName) - 1);
            sourceLifName[sizeof(sourceLifName) - 1] = 0;
...
//            reg = &trTextureRegistry[sortList->textureList[0]];
            ...
            if (trModernTryLoadLoosePng(reg, lifFile, colorInfo,
                                          scalar0, scalar1, sourceLifName))
            {
                trClearPending(sortList->textureList[index]);
                memFree(colorInfo);
                memFree(lifFile);
        ...
        {                                                   //texture slot not free
            ...
            {                                               //if pending a load
                ...
                {                                           //else the list search measurment worked
                    ...
                    /* A loose PNG is an explicit per-path override.  Do not
                       collapse it to a textures.ll shared parent or the loose
                       replacement could be bypassed by the parent's LIF. */
                    if (trModernLoosePngExists(reg->fileName))
                    {
                        pSharedFrom = NULL;
                    }
                    if (pSharedFrom == NULL)
...
abortloading:
    ...
    if (!hrAbortLoadingGame)
    {
        meshResolveModernSurfaceTextures();
    }
#endif
#if MEM_ANALYSIS
    memAnalysisCreate();
