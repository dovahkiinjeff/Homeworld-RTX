...
/* OpenGL infinite-far projection. This retains the normal nonlinear depth
   distribution near the camera but removes the far clipping plane entirely. */
void rgluPerspectiveInfinite(GLfloat fovy, GLfloat aspect, GLfloat zNear)
{
    GLfloat cotangent = 1.0f / (GLfloat)tan(fovy * M_PI / 360.0);
    GLfloat projection[16] = {0.0f};
