...
sdword mouseCursorScalePercent = 100;
uword mouseButtons;
...
static bool32 mouseGLInitialized;
...
static sdword mouseWheelStepsPending = 0;

void mouseWheelStepsAdd(sdword steps)
{
    /* A generous guard protects the old 32-bit input path from a malformed
       event without discarding any plausible mouse or precision-wheel burst. */
    const sdword maximumPendingSteps = 256;
    mouseWheelStepsPending += steps;
    if (mouseWheelStepsPending > maximumPendingSteps)
    {
        mouseWheelStepsPending = maximumPendingSteps;
    }
    else if (mouseWheelStepsPending < -maximumPendingSteps)
    {
        mouseWheelStepsPending = -maximumPendingSteps;
    }
}

sdword mouseWheelStepsPeek(void)
{
    return mouseWheelStepsPending;
}

sdword mouseWheelStepsTake(void)
{
    sdword steps = mouseWheelStepsPending;
    mouseWheelStepsPending = 0;
    return steps;
}
...
static udword tex_HDCursor[end_cursor_type];
static bool8 tex_HDCursorAttempted[end_cursor_type];
...
void mouseReset(void)
{
    ...
    {
        sdword cursorIndex;
        for (cursorIndex = 0; cursorIndex < end_cursor_type; ++cursorIndex)
        {
            if (tex_HDCursor[cursorIndex] != 0)
            {
                trRGBTextureDelete(tex_HDCursor[cursorIndex]);
                tex_HDCursor[cursorIndex] = 0;
            }
            tex_HDCursorAttempted[cursorIndex] = FALSE;
        }
    }
...
void mouseDrawTriangle(void)
{
    ...
    sdword scalePercent = mouseCursorScalePercent;
    if (scalePercent < 25) scalePercent = 25;
    if (scalePercent > 100) scalePercent = 100;

    mouseTri.x0 = mouseCursorXPosition + 0;
    mouseTri.y0 = mouseCursorYPosition + 0;
    mouseTri.x1 = mouseCursorXPosition + uiScaleSize(4) * scalePercent / 100;
    mouseTri.y1 = mouseCursorYPosition + uiScaleSize(16) * scalePercent / 100;
    mouseTri.x2 = mouseCursorXPosition + uiScaleSize(14) * scalePercent / 100;
    mouseTri.y2 = mouseCursorYPosition + uiScaleSize(7) * scalePercent / 100;
    mouseTri2 = mouseTri;
    mouseTri2.x2++;
...
/* Preserve the exact cursor-to-art mapping used by the 1999 game.  Several
   manager actions deliberately share the dialog cursor, and salvage and
   special attack deliberately share the activation cursor. */
static const char *mouseHDCursorFile(mouseCursor cursorType)
{
    switch (cursorType)
    {
        case normal_mouse:
        case movement:              return "cursor.png";
        case add_ships:             return "add.png";
        case focus_no_select:       return "eye.png";
        case band_box_attack:       return "attack.png";
        case force_attack:          return "exclaim.png";
        case m_guard:               return "shield.png";
        case special_attack:
        case special_op_activate:
        case salvage:               return "special_activate.png";
        case special_op_deactivate: return "special_deactivate.png";
        case gui:
        case research:
        case sensors:
        case build:                 return "dialog.png";
        case resource:              return "resource.png";
        case docking:
        case small_docking:         return "dock.png";
        case support:               return "support.png";
        case traders:               return "trader.png";
        default:                    return NULL;
    }
}

static void *mouseLoadHDFile(const char *fileName, size_t *fileSize)
{
    char path[2048];
    ...
    *fileSize = 0;
    if (basePath != NULL)
    {
        for (index = 0; index < 2 && fileData == NULL; ++index)
        {
            snprintf(path, sizeof(path), "%s%s%s", basePath,
                     relativeCandidates[index], fileName);
    ...
    if (fileData == NULL)
    {
        snprintf(path, sizeof(path), "%s%s", relativeCandidates[2],
                 fileName);
        fileData = SDL_LoadFile(path, fileSize);
    }
    return fileData;
...
static udword mouseEnsureHDCursor(mouseCursor cursorType)
{
    const char *fileName;
    void *fileData;
    size_t fileSize;
    ...
    if (cursorType < 0 || cursorType >= end_cursor_type)
        return 0;
    if (tex_HDCursor[cursorType] != 0)
        return tex_HDCursor[cursorType];
    if (tex_HDCursorAttempted[cursorType])
        return 0;
    tex_HDCursorAttempted[cursorType] = TRUE;
    fileName = mouseHDCursorFile(cursorType);
    if (fileName == NULL)
        return 0;

    fileData = mouseLoadHDFile(fileName, &fileSize);
    ...
    if (texture != 0)
    {
        ...
        tex_HDCursor[cursorType] = texture;
    }
    return texture;
}

static GLuint mouseLegacyTexture(mouseCursor cursorType)
{
    switch (cursorType)
    {
        case normal_mouse:
        case movement:              return tex_Normal;
        case add_ships:             return tex_AddShips;
        case focus_no_select:       return tex_FocusNoSelect;
        case band_box_attack:       return tex_BandBoxAttack;
        case force_attack:          return tex_ForceAttack;
        case m_guard:               return tex_Guard;
        case special_attack:        return tex_SpecialAttack;
        case special_op_activate:   return tex_SpecialOpActivate;
        case special_op_deactivate: return tex_SpecialOpDeactivate;
        case gui:                   return tex_GUI;
        case resource:              return tex_Resource;
        case docking:
        case small_docking:         return tex_Docking;
        case research:              return tex_Research;
        case sensors:               return tex_Sensors;
        case build:                 return tex_Build;
        case salvage:               return tex_Salvage;
        case support:               return tex_Support;
        case traders:               return tex_Traders;
        default:                    return 0;
    }
}

static void mouseDrawModernCursor(mouseCursor cursorType)
{
    lifheader *legacy = mouseGetLIF();
    udword texture = mouseEnsureHDCursor(cursorType);
    rectangle rect;
    sdword scalePercent;
    sdword drawWidth;
    sdword drawHeight;
    ...
    if (texture == 0)
        texture = mouseLegacyTexture(cursorType);
    if (texture == 0 || legacy == NULL)
    {
        mouseDrawTriangle();
        return;
    }

    scalePercent = mouseCursorScalePercent;
    if (scalePercent < 25) scalePercent = 25;
    if (scalePercent > 100) scalePercent = 100;
    drawWidth = uiScaleSize(legacy->width) * scalePercent / 100;
    drawHeight = uiScaleSize(legacy->height) * scalePercent / 100;
    if (drawWidth < 1) drawWidth = 1;
    if (drawHeight < 1) drawHeight = 1;
    rect.x0 = mouseCursorXPosition;
    rect.y0 = mouseCursorYPosition;
    rect.x1 = rect.x0 + drawWidth;
    rect.y1 = rect.y0 + drawHeight;
    trRGBTextureMakeCurrent(texture);
...
void mouseDraw(void)
{
    ...
    if (((feStackIndex > 0) && (!spHyperspaceRollCallScreen) &&
         !(smSensorsActive && !smFleetIntel)) ||
        ((bitTest(mouseCursorMode, MCM_Pointmode)) && (mouseCursorType != focus_no_select)))
    {
        /* RTX-0069: ordinary Sensor Manager is now a tactical viewport.
           Do not let the legacy FE-stack rule erase its live modifier cursor
           (Ctrl attack, Ctrl+Shift force attack, Alt focus, Z special, guard).
           Fleet Intel remains a true modal FE screen and keeps the old rule. */
        mouseCursorType = normal_mouse;
    }

    if (mouseCursorType == no_mouse || mouseCursorType == rotate_camera ||
        mouseCursorType == zoom_camera)
    {
        goto finish;
    }
    if (mouseCursorType == special_attack && mousehold == normal_mouse)
    {
        mousehold = special_attack;
        mouseholdexpire = universe.totaltimeelapsed + (real32)MC_MOUSEHOLDTIME;
    }
    mouseDrawModernCursor(mouseCursorType);
...
void mouseCursorTextDraw(void)
{
    ...
    if (((!mrRenderMainScreen) && !(smSensorsActive && !smFleetIntel)) ||
        (!gameIsRunning) || (mrWhiteOut) || (nisIsRunning) || (keyIsHit(RMOUSE_BUTTON)))
    {
        return;
    }
