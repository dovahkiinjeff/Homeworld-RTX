...
bool32 salCapAreEnoughSalvagersTargettingThisTarget(Ship *ship,SpaceObjRotImpTargGuidanceShipDerelict *target);
...
static bool32 salCapIsExceptionalMothership(
    SpaceObjRotImpTargGuidanceShipDerelict *target)
{
    ...
    return ((Ship *)target)->shiptype == P1Mothership ||
           ((Ship *)target)->shiptype == P2Mothership;
}

static sdword salCapExceptionalRequiredSalvagers(
    SpaceObjRotImpTargGuidanceShipDerelict *target)
{
    sdword required;
    ...
    required = (((Ship *)target)->shiptype == P2Mothership) ?
               P2_MOTHERSHIP_SALVAGERS_REQUIRED :
               P1_MOTHERSHIP_SALVAGERS_REQUIRED;

    if(target->salvageInfo != NULL &&
       target->staticinfo->salvageStaticInfo != NULL &&
       target->staticinfo->salvageStaticInfo->numSalvagePoints < required)
    {
        required = target->staticinfo->salvageStaticInfo->numSalvagePoints;
    }

    return required;
}

static sdword salCapExceptionalAttachedSalvagers(
    Ship *ship, SpaceObjRotImpTargGuidanceShipDerelict *target)
{
    ...
    while(node != NULL)
    {
        Ship *candidate = (Ship *)listGetStructOfNode(node);

        if(candidate->shiptype == SalCapCorvette &&
           candidate->playerowner == ship->playerowner &&
           candidate->clampInfo != NULL &&
           candidate->clampInfo->host == (SpaceObjRotImpTarg *)target)
        {
            count++;
        }
...
static bool32 salCapSuperShipIntercept(
    Ship *ship, SpaceObjRotImpTargGuidanceShipDerelict *target)
{
    if (!singlePlayerGame || !opSuperSalvagers ||
        ship == NULL || target == NULL ||
    ...
    /* Super Salvagers use the same full-speed intercept contract for every
       salvageable ship class.  The target-selection rules still decide what
       can actually be salvaged; this helper only removes class-specific
       approach behavior once a valid ship target has been assigned. */
    return TRUE;
}
...
bool32 handOffTargetToDockWith(Ship *ship);
...
void SalCapCorvetteAttackPassive(Ship *ship,Ship *target,bool32 rotate)
{
...
bool32 SalCapCorvetteSpecialActivate(Ship *ship)
{
    vector place;
...
/* Super Salvagers should not inherit the slow multi-stage capital docking
   approach.  Pursue the assigned salvage point at full speed, predict the
   next physics-step crossing, then snap the final few units to the authored
   point before the existing clamp code freezes the relative transform. */
static sdword salCapSuperFlyToSalvagePoint(
    Ship *ship, SpaceObjRotImpTargGuidanceShipDerelict *target)
{
    SalCapCorvetteSpec *spec = (SalCapCorvetteSpec *)ship->ShipSpecifics;
    ...
    vector travelHeading;
    real32 range;
    real32 closingSpeed;
    real32 attachDistance;
    real32 sweptAttachDistance;

    if (target == NULL || target->salvageInfo == NULL ||
    ...
    salvagestaticpoint =
        target->salvageInfo->salvagePoints[spec->salvageIndex].salvageStaticPoint;
    matMultiplyMatByVec(&coneheadingInWorldCoordSys,
                        &target->rotinfo.coordsys,
                        &salvagestaticpoint->conenormal);
    matMultiplyMatByVec(&conepositionInWorldCoordSys,
                        &target->rotinfo.coordsys,
                        &salvagestaticpoint->position);
    vecAddTo(conepositionInWorldCoordSys,target->posinfo.position);
    vecScalarMultiply(desiredHeading,coneheadingInWorldCoordSys,-1.0f);

    vecSub(toPoint,conepositionInWorldCoordSys,ship->posinfo.position);
    range = fsqrt(vecMagnitudeSquared(toPoint));
    travelHeading = toPoint;
    if (range > 0.001f)
    {
        vecScalarMultiply(travelHeading,travelHeading,1.0f / range);
    }
    else
    {
        travelHeading = desiredHeading;
    }

    attachDistance =
        ship->staticinfo->staticheader.staticCollInfo.approxcollspheresize *
        0.55f + 16.0f;
    if (attachDistance < 38.0f) attachDistance = 38.0f;
    if (attachDistance > 90.0f) attachDistance = 90.0f;

    vecSub(relativeVelocity,ship->posinfo.velocity,target->posinfo.velocity);
    closingSpeed = vecDotProduct(relativeVelocity,travelHeading);
    sweptAttachDistance = attachDistance;
    if (closingSpeed > 0.0f)
    {
        sweptAttachDistance += closingSpeed * universe.phystimeelapsed * 1.35f;
    }

    if (range <= sweptAttachDistance)
    {
        /* Preserve the authored capital salvage-point placement.  The snap is
           only the last predicted physics-step distance and prevents a fast
           salvager from crossing the point, reversing, and rubber-banding. */
        ship->posinfo.position = conepositionInWorldCoordSys;
        ship->posinfo.velocity = target->posinfo.velocity;
        aitrackHeadingWithFlags(
            ship,&desiredHeading,0.70f,AITRACKHEADING_IGNOREUPVEC);
        return TRUE;
    }

    aishipFlyToPointAvoidingObjsWithVel(
        ship,&conepositionInWorldCoordSys,
        AISHIP_FastAsPossible | AISHIP_PointInDirectionFlying,
        -tacticsGetShipsMaxVelocity(ship),
        &target->posinfo.velocity);
    aitrackHeadingWithFlags(
        ship,&desiredHeading,0.70f,AITRACKHEADING_IGNOREUPVEC);
    return FALSE;
}
...
bool32 isThereAnotherTargetForMe(Ship *ship,SelectAnyCommand *targets)
{
    ...
    for(i=0;i<targets->numTargets;i++)
    {
        ...
        if(t->objtype == OBJ_ShipType)
        {
            ...
            case P1Mothership:
            case P2Mothership:
                /* CAPTURE / BUILD ALL is the explicit opt-in for exceptional
                   Turanic / Kadeshi production-mothership capture. */
                if (singlePlayerGame && opCaptureBuildAll &&
                    ship->playerowner == universe.curPlayerPtr)
                {
                    break;
                }
    ...
    *flyandwait = FALSE;
    ...
    for(i=0;i<targets->numTargets;i++)
    {
        ...
        if(t->objtype == OBJ_ShipType)
        {
            ...
            case P1Mothership:
            case P2Mothership:
                /* Exceptional production-ship capture is opt-in. */
                if (!singlePlayerGame || !opCaptureBuildAll ||
                    ship->playerowner != universe.curPlayerPtr)
                {
                    continue;
...
{
    bool32 spm3AlreadySalvaged = FALSE;   // TRUE if the player has already salvaged a frigate in Mission 3
...
sdword salCapGetAndBusySalvageIndex(Ship *ship,SpaceObjRotImpTargGuidanceShipDerelict *target)
{
    ...
    if(target->salvageInfo != NULL)
    {
        if (salCapSuperShipIntercept(ship,target))
        {
            sdword bestIndex = NO_INDEX;
            real32 bestDistanceSquared = 1.0e30f;

            /* Super Salvagers take the nearest free authored salvage point.
               The original first-free ordering can send a fast corvette to
               the far side of a capital ship before it is allowed to latch. */
            for (i = 0;i<target->salvageInfo->numSalvagePoints;i++)
            {
                vector pointInWorld;
                vector toPoint;
                real32 distanceSquared;
                SalvageStaticPoint *staticPoint;
                ...
                staticPoint = salvagePoint->salvageStaticPoint;
                matMultiplyMatByVec(&pointInWorld,&target->rotinfo.coordsys,
                                    &staticPoint->position);
                vecAddTo(pointInWorld,target->posinfo.position);
                vecSub(toPoint,pointInWorld,ship->posinfo.position);
                distanceSquared = vecMagnitudeSquared(toPoint);
            ...
            if (bestIndex != NO_INDEX)
            {
                target->salvageInfo->salvagePoints[bestIndex].busy = TRUE;
...
sdword salCapFitCarrier(Ship *ship,SpaceObjRotImpTargGuidanceShipDerelict *target)
{
    ...
    if(salCapIsExceptionalMothership(target))
    {
        /* Exceptional production motherships are full capital-scale prizes.
           Always route retrofit/deposit through the player's mothership path. */
        return FALSE;
    }
...
/*-----------------------------------------------------------------------------
    ...
    Return      :
----------------------------------------------------------------------------*/
bool32 SalCapCorvetteSpecialTarget(Ship *ship, void *custom)
{
...
reachedit:
    ...
    case SAL_FLYTOTARGET:
        ...
        if (salCapSuperShipIntercept(ship, spec->target))
        {
            /* Every valid ship target now gets the Super Salvager approach:
               no target avoidance and no legacy distance-based speed taper. */
            spec->noAvoid = TRUE;
        }
        ...
        if (salCapSuperShipIntercept(ship, spec->target))
        {
            /* Skip the normal fly-to-distance gate for every ship class.
               Targets with authored salvage points reserve one immediately;
               targets without them use the direct clamp path. */
            ship->specialFlags |= SPECIAL_BrokenFormation;
            spec->salvageState = spec->target->salvageInfo != NULL
                ? SAL_DOCKWITHLIGHTS1 : SAL_DOCKWITHOUTLIGHTS1;
            break;
        }
    ...
    case SAL_DOCKWITHOUTLIGHTS1:
        ...
        }
        {
            ...
            superShip = salCapSuperShipIntercept(ship, spec->target);
            ...
            if (superShip)
            {
                vector relativeVelocity;
                real32 closingSpeed;
                real32 attachDistance = salcapcorvettestatics->noLightClampingDistance;
                real32 hullContactDistance =
                    ship->staticinfo->staticheader.staticCollInfo.approxcollspheresize *
                    1.20f + 12.0f;
                real32 sweptAttachDistance;

                /* Clamp before the collision hulls meet.  The old code waited
                   for a very small range AND a 0.98 heading lock; a fast
                   corvette could cross that window, fail the orientation test,
                   then have its guidance reverse direction on the next tick. */
                if (attachDistance < hullContactDistance)
                {
                    attachDistance = hullContactDistance;
                }

                /* Include the distance that will be closed during the next
                   physics step.  This is the anti-rubber-band piece: a Super
                   Salvager latches on the frame before it would overshoot the
                   capture envelope, rather than crossing the target and being
                   told to reverse at full thrust on the following update. */
                vecSub(relativeVelocity,ship->posinfo.velocity,
                       spec->target->posinfo.velocity);
                closingSpeed = vecDotProduct(relativeVelocity,heading);
                sweptAttachDistance = attachDistance;
                if (closingSpeed > 0.0f)
                {
                    sweptAttachDistance += closingSpeed *
                        universe.phystimeelapsed * 1.25f;
                }

                if (range <= sweptAttachDistance)
                {
                    clampObjToObj((SpaceObjRotImpTargGuidance *)spec->target,
                                  (SpaceObjRotImpTargGuidance *)ship);
                ...
                /* Force a full-speed pursuit right to the latch envelope.
                   AISHIP_FastAsPossible removes the distance->velocity taper;
                   the negative limit supplies a minimum desired speed equal
                   to the Super Salvager's current max speed.  Target velocity
                   is included so moving ships of every class are intercepted
                   rather than chased from a stale position. */
                aishipFlyToPointAvoidingObjsWithVel(
                    ship, &spec->target->posinfo.position,
                    AISHIP_FastAsPossible | AISHIP_PointInDirectionFlying,
                    -tacticsGetShipsMaxVelocity(ship),
                    &spec->target->posinfo.velocity);
                aitrackHeadingWithFlags(
                    ship,&heading,0.90f,AITRACKHEADING_IGNOREUPVEC);
                break;
            }
    ...
    case SAL_WL_FLYTOWITHINCONE:
        ...
        else if(salCapFlyToWithInCone(ship,spec->target))
        {
            spec->noAvoid = TRUE;
            spec->salvageState = SAL_WL_FLYTOCONEORIGIN;
        }
    ...
    case SAL_WL_FLYTOCONEORIGIN:
        ...
        if ((salCapSuperShipIntercept(ship, spec->target) &&
             salCapSuperFlyToSalvagePoint(ship,spec->target)) ||
            (!salCapSuperShipIntercept(ship, spec->target) &&
             salCapFlyToConeOrigin(ship,spec->target)))
        {
...
//        speechEvent(ship, STAT_SCVette_TargetAcquired, 0);
        ...
        else if(spec->target->objtype == OBJ_ShipType)
        {
            ...
            if(salCapIsExceptionalMothership(spec->target))
            {
                /* Exceptional mothership capture is clamp-count driven.
                   Never enter the normal salvage damage phase: campaign AI
                   can interpret that damage as a retreat/escape trigger. */
                spec->salvageState = SAL_SALVAGE1;
                break;
            }
    ...
    case SAL_DO_DAMAGE:
        ...
        if(spec->target->objtype == OBJ_ShipType)
        {
            ...
            case P1Mothership:
            case P2Mothership:
                /* Exceptional motherships must never re-enter the damage
                   gate.  This also repairs an in-progress save that was
                   already in SAL_DO_DAMAGE before this support existed. */
                spec->salvageState = SAL_SALVAGE1;
                break;
            default:
                break;
    ...
    case SAL_SALVAGE1:
        ...
        if(ship->shiptype != JunkYardDawg)
        {
            ...
            if(salCapIsExceptionalMothership(spec->target))
            {
                /* Do not try to physically tow a Turanic/Kadeshi mothership
                   through a normal salvage docking corridor.  Count actual
                   player Salvage Corvettes clamped to the hull; when five are
                   attached, hand the prize directly to the existing captured-
                   ship retrofit queue.  That path preserves native ship type,
                   race and player ownership. */
                {
                    sdword required = salCapExceptionalRequiredSalvagers(spec->target);
                    sdword attached = salCapExceptionalAttachedSalvagers(ship,spec->target);

                    if(spec->target->salvageInfo != NULL)
                    {
                        spec->target->salvageInfo->numNeededForSalvage =
                            (attached < required) ? (required - attached) : 0;
                    }
                ...
                salCapHarvestTarget(spec->target,spec->dockwith);
                return TRUE;
            }
